...
validate_options(Values) ->
    smf_core_config:validate_options(fun validate_option/2, Values, ?ClusterDefaults).
...
handle_event({call, From}, {start, Config}, ready, Data) ->
    application:set_env([{smf_cluster, maps:to_list(Config)}]),
...
handle_event(info, {'EXIT', Pid, ok}, startup,
    ...
    ?LOG(info, "smf_core: ready to process requests, cluster started in ~w ms",
