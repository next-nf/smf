...
-dialyzer({nowarn_function, [prepare_request/4]}).
...
initialize_service(_ServiceId, #{function := Function}) ->
	...
	#{'Acct-Application-Id' => ?DIAMETER_APP_ID_RF,
	  ...
	  application => [{alias, ?APP},
			  ...
			  {module, ?MODULE}]},
    smf_aaa_diameter_srv:register_service(Function, SvcOpts),
    {ok, []}.
...
%%%===================================================================
%%% smf_aaa_diameter_srv wrapper
%%%===================================================================

send_request(Request, Config) ->
    smf_aaa_diameter_srv:send_request(?MODULE, ?APP, Request, Config).
...
await_response(Promise, Session, Events, State, Opts) ->
    Msg = smf_aaa_diameter_srv:await_response(Promise),
...
peer_down(SvcName, {_, #diameter_caps{origin_host = {OH, _}}} = Peer, State) ->
    smf_aaa_diameter_srv:peer_down(?MODULE, SvcName, Peer),
    ?LOG(info, "Diameter peer FSM for ~p changed to state DOWN", [OH]),
    State.

%% pick_peer/5
pick_peer([], RemoteCandidates, SvcName, _State, CallOpts) ->
    smf_aaa_diameter_srv:pick_peer(RemoteCandidates, SvcName, CallOpts);
pick_peer(LocalCandidates, _, SvcName, _State, CallOpts) ->
    smf_aaa_diameter_srv:pick_peer(LocalCandidates, SvcName, CallOpts).
...
prepare_request(#diameter_packet{msg = ['ACR' = T | Avps]} = Pkt0, SvcName,
  ...
  when is_map(Avps) ->
    ...
    Pkt = smf_aaa_diameter_srv:prepare_request(
	    Pkt0#diameter_packet{msg = Msg}, SvcName, Peer, CallOpts),
    smf_aaa_diameter_srv:start_request(Pkt, SvcName, Peer);
...
handle_answer(#diameter_packet{msg = Msg, errors = Errors},
  ...
  when length(Errors) /= 0 ->
    ...
    ok = smf_aaa_diameter_srv:finish_request(SvcName, Peer),
    case Msg of
...
handle_error(Reason, _Request, SvcName, Peer, CallOpts) ->
    ok = smf_aaa_diameter_srv:finish_request(SvcName, Peer),
    smf_aaa_diameter_srv:retry_request(Reason, SvcName, Peer, CallOpts).
...
validate_option(termination_cause_mapping, Value) ->
    smf_aaa_diameter:validate_termination_cause_mapping(Value);
...
to_session(_, 'Acct-Interim-Interval', [Interim], {Session, Events})
  when is_integer(Interim), Interim > 0 ->
    Trigger = smf_aaa_session:trigger(?APP, 'Offline', periodic, Interim),
    {Session, smf_aaa_session:ev_set(Trigger, Events)};
...
from_session(Key, Value, Avps)
  when Key =:= '3GPP-PDP-Type';
       ...
       Key =:= '3GPP-RAT-Type' ->
    optional([?SI_PSI, Key], smf_aaa_diameter:'3gpp_from_session'(Key, Value), Avps);
...
from_session(Key, Value, Avps)
  when Key =:= 'User-Location-Info';
    ...
    optional([?SI_PSI, '3GPP-User-Location-Info'],
	     smf_aaa_diameter:'3gpp_from_session'(Key, Value), Avps);
...
from_session('3GPP-Charging-Id' = Key, Value, Avps0) ->
    Avps1 = optional([?SI_PSI, Key], smf_aaa_diameter:'3gpp_from_session'(Key, Value), Avps0),
...
from_session('QoS-Information' = Key, Value, Avps) ->
    optional([?SI_PSI, Key], smf_aaa_diameter:qos_from_session(Value), Avps);
...
from_prev_session('QoS-Information' = Key, Value, Avps) ->
    optional([?SI_TP_PREV_PSI, Key], smf_aaa_diameter:qos_from_session(Value), Avps);
...
from_prev_session(Key, Value, Avps)
  when Key =:= '3GPP-SGSN-MCC-MNC';
       Key =:= '3GPP-MS-TimeZone';
       Key =:= '3GPP-RAT-Type' ->
    optional([?SI_TP_PREV_PSI, Key], smf_aaa_diameter:'3gpp_from_session'(Key, Value), Avps);
...
stop_indicator(?'DIAMETER_RF_ACCOUNTING-RECORD-TYPE_STOP_RECORD', Avps) ->
    optional([?SI_PSI, '3GPP-Session-Stop-Indicator'],
	     smf_aaa_diameter:'3gpp_from_session'('3GPP-Session-Stop-Indicator', true), Avps);
