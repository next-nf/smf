...
initialize_function(Id, #{'Origin-Host' := {OriginHost, Addr},
    ...
    ProductName = application:get_env(smf_aaa, product_name, "erGW-AAA"),
    ...
    SvcOpts = merge_svc(SvcOpts0, smf_aaa_diameter_srv:get_service_opts(Id)),
...
validate_function(Opts) ->
    smf_aaa_config:validate_options(fun validate_function/2, Opts, ?DefaultFunctionOptions).

validate_function(transports, Opts) when ?non_empty_opts(Opts) ->
    lists:map(smf_aaa_config:validate_options(
...
'3gpp_from_session'(Key, Value)
  when (Key =:= '3GPP-Charging-Gateway-Address' orelse
	...
	Key =:= '3GPP-GGSN-Address') andalso
       ?IS_IPv4(Value) ->
    smf_aaa_3gpp_dict:ip2bin(Value);
...
'3gpp_from_session'(Key, Value)
  when Key =:= '3GPP-PDP-Type';
    ...
    smf_aaa_3gpp_dict:encode(Key, Value);
