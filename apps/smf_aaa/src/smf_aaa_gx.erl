...
-include("include/smf_aaa_gx.hrl").
...
prepare_request(#diameter_packet{msg = ['CCR' = T | Avps]} = Pkt0, SvcName,
  ...
  when is_map(Avps) ->
    ...
    SF = case catch(smf_aaa_diameter_srv:is_first_request(?MODULE, SvcName, Peer)) of
	     false ->
		 [];
	     _ ->
...
from_session('Framed-IPv6-Prefix', Prefix, Avps) ->
    Avps#{'Framed-IPv6-Prefix' => smf_aaa_diameter:decode_ipv6prefix(Prefix)};
...
from_session(Key, Value, Avps)
  when Key =:= '3GPP-Selection-Mode';
       ...
       Key =:= '3GPP-RAT-Type' ->
    Avps#{Key => [smf_aaa_diameter:'3gpp_from_session'(Key, Value)]};

%% '3GPP-User-Location-Info'
from_session(Key, Value, Avps)
  when Key =:= 'User-Location-Info';
       Key =:= '3GPP-User-Location-Info' ->
    Avps#{'3GPP-User-Location-Info' => [smf_aaa_diameter:'3gpp_from_session'(Key, Value)]};
...
from_session('QoS-Information' = Key, Value, Avps) ->
    Avps#{Key => [smf_aaa_diameter:qos_from_session(Value)]};
