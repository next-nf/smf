...
setopts(Opts0) when is_map(Opts0)->
    Opts = maps:map(fun smf_aaa_config:validate_option/2, Opts0),
    maps:map(application:set_env(smf_aaa, _, _), Opts),
    ok;
setopts(Opts) when is_list(Opts) ->
    setopts(smf_aaa_config:to_map(Opts)).

setopt(rate_limit = _Opt, {Key, Value0}) ->
    Value = smf_aaa_config:validate_rate_limit(Key, Value0),
    Limits = application:get_env(smf_aaa, rate_limits, #{}),
    application:set_env(smf_aaa, rate_limits, maps:put(Key, Value, Limits));
setopt(Opt, Value0) ->
    Value = smf_aaa_config:validate_option(Opt, Value0),
    application:set_env(smf_aaa, Opt, Value).

add_function(Name, Opts0) ->
    #{handler := Handler} =
	Opts = smf_aaa_config:validate_function(Name, Opts0),
    add_config(?FUNCTION_KEY, Name, Opts),
    {ok, SupSpec} = Handler:initialize_function(Name, Opts),
    smf_aaa_sup:start_childs(SupSpec),
    ok.

add_handler(Handler, Opts0) ->
    Opts = smf_aaa_config:validate_handler(Handler, Opts0),
...
add_service(Name, Opts0) ->
    #{handler := Handler} =
	Opts = smf_aaa_config:validate_service(Name, Opts0),
...
add_application(Name, Opts0) ->
    Opts = smf_aaa_config:validate_app(Name, Opts0),
    add_config(?APP_KEY, Name, Opts).

get_function(Name) ->
    Functions = application:get_env(smf_aaa, ?FUNCTION_KEY, undefined),
    maps:get(Name, Functions, undefined).

get_handler(Name) ->
    Handlers = application:get_env(smf_aaa, ?HANDLER_KEY, undefined),
    maps:get(Name, Handlers, undefined).

get_service(Name) ->
    Services = application:get_env(smf_aaa, ?SERVICE_KEY, #{}),
    maps:get(Name, Services, #{}).

get_application(Name) ->
    Apps = application:get_env(smf_aaa, ?APP_KEY, #{}),
