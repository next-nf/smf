...
validate_handler(Opts) ->
    smf_aaa_config:validate_options(fun validate_option/2, Opts, ?DefaultOptions).
...
validate_option(server, Opts0) when ?is_opts(Opts0)->
    Opts = smf_aaa_config:to_map(Opts0),
    mandatory_keys([host, port, secret], Opts),
    smf_aaa_config:validate_options(fun validate_server_opt/2, Opts, []);
...
validate_termination_cause_mapping(Opts) when ?is_opts(Opts) ->
    smf_aaa_config:validate_options(fun validate_termination_cause_mapping/2, Opts, ?DEFAULT_TERMINATION_CAUSE_MAPPING);
...
to_session(_, 'Acct-Interim-Interval', Interim, {Session, Events, State})
  when is_integer(Interim), Interim > 0 ->
    Trigger = smf_aaa_session:trigger(accounting, 'IP-CAN', periodic, Interim),
    {Session, smf_aaa_session:ev_set(Trigger, Events), State};
...
session_options('3GPP-PDP-Type' = Key, Value, Attrs) ->
    [{?X_3GPP_PDP_Type, smf_aaa_3gpp_dict:encode(Key, Value)}|Attrs];
...
session_options('3GPP-GPRS-Negotiated-QoS-Profile' = Key, Value, Attrs) ->
    [{?X_3GPP_GPRS_Negotiated_QoS_profile, smf_aaa_3gpp_dict:encode(Key, Value)}|Attrs];
...
session_options('3GPP-IMSI-MCC-MNC' = Key, Value, Attrs) ->
    [{?X_3GPP_IMSI_MCC_MNC, smf_aaa_3gpp_dict:encode(Key, Value)}|Attrs];
session_options('3GPP-GGSN-MCC-MNC' = Key, Value, Attrs) ->
    [{?X_3GPP_GGSN_MCC_MNC, smf_aaa_3gpp_dict:encode(Key, Value)}|Attrs];
session_options('3GPP-NSAPI' = Key, Value, Attrs) ->
    [{?X_3GPP_NSAPI, smf_aaa_3gpp_dict:encode(Key, Value)}|Attrs];
session_options('3GPP-Session-Stop-Indicator' = Key, Value, Attrs) ->
    [{?X_3GPP_Session_Stop_Indicator, smf_aaa_3gpp_dict:encode(Key, Value)}|Attrs];
session_options('3GPP-Selection-Mode' = Key, Value, Attrs) ->
    [{?X_3GPP_Selection_Mode, smf_aaa_3gpp_dict:encode(Key, Value)}|Attrs];
session_options('3GPP-Charging-Characteristics' = Key, Value, Attrs) ->
    [{?X_3GPP_Charging_Characteristics, smf_aaa_3gpp_dict:encode(Key, Value)}|Attrs];
...
session_options('3GPP-IPv6-DNS-Servers' = Key, Value, Attrs) ->
    [{?X_3GPP_IPv6_DNS_Servers, smf_aaa_3gpp_dict:encode(Key, Value)}|Attrs];
session_options('3GPP-SGSN-MCC-MNC' = Key, Value, Attrs) ->
    [{?X_3GPP_SGSN_MCC_MNC, smf_aaa_3gpp_dict:encode(Key, Value)}|Attrs];
session_options('3GPP-Teardown-Indicator' = Key, Value, Attrs) ->
    [{?X_3GPP_Teardown_Indicator, smf_aaa_3gpp_dict:encode(Key, Value)}|Attrs];
session_options('3GPP-IMEISV', Value, Attrs) ->
    [{?X_3GPP_IMEISV, Value}|Attrs];
session_options('3GPP-RAT-Type' = Key, Value, Attrs) ->
    [{?X_3GPP_RAT_Type, smf_aaa_3gpp_dict:encode(Key, Value)}|Attrs];
...
session_options('User-Location-Info' = Key, Value, Attrs) when is_map(Value) ->
    [{?X_3GPP_User_Location_Info, smf_aaa_3gpp_dict:encode(Key, Value)}|Attrs];
session_options('3GPP-MS-TimeZone' = Key, Value, Attrs) ->
    [{?X_3GPP_MS_TimeZone, smf_aaa_3gpp_dict:encode(Key, Value)}|Attrs];
...
to_session_opts(_Attr, Key, Value, SOpts)
  when
    ...
    SOpts#{Key => smf_aaa_3gpp_dict:decode(Key, Value)};
