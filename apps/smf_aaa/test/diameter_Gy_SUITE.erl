...
ocs_hold_update_timeout(Config) ->
    ...
    ?equal([{smf_aaa_ro, ocs_hold, 1}, {smf_aaa_ro, started, 0}], get_session_stats()),
...
diameter_metrics(_Config) ->
    ?equal(ok, prometheus_registry:register_collector(smf_aaa, smf_aaa_prometheus_collector)),
    Metrics = prometheus_text_format:format(smf_aaa),
    ct:pal("Metrics: ~s", [Metrics]),
    ?match({match, _}, re:run(Metrics, "smf_aaa_diameter_outstanding_requests")),
    ?match({match, _}, re:run(Metrics, "smf_aaa_diameter_available_tokens")),
    ?match({match, _}, re:run(Metrics, "smf_aaa_diameter_no_tokens_available_total")),
    ?match({match, _}, re:run(Metrics, "smf_aaa_diameter_no_capacity_left_total")).
