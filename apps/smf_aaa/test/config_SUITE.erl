...
-define(CONFIG,
	#{functions => ?DIAMETER_FUNCTION,
	  handlers =>
		...
		smf_aaa_radius => ?RADIUS_ACCT_CONFIG,
		smf_aaa_nasreq => ?DIAMETER_CONFIG,
		smf_aaa_rf     => ?RF_CONFIG,
		smf_aaa_ro     => ?RF_CONFIG},
	  services =>
	      #{<<"Default">> =>
		...
		<<"RF-Service">> =>
		    maps:put(handler, 'smf_aaa_rf', ?RF_CONFIG),
		<<"RO-Service">> =>
		    maps:put(handler, 'smf_aaa_ro', ?RF_CONFIG)},
	  apps =>
	      #{<<"RADIUS-Application">> =>
...
toplevel(_Config)  ->
    ...
    ValF = fun(Opts) ->
		   smf_aaa_config:validate_options(
		     fun smf_aaa_config:validate_option/2, Opts, [])
	   end,
...
rate_limits(_Config)  ->
    ...
    ValF = fun smf_aaa_config:validate_rate_limit/2,
...
function(_Config)  ->
    ...
    ValF = fun smf_aaa_config:validate_function/2,
...
static_handler(_Config)  ->
    Cfg = ?STATIC_CONFIG,
    ValF = fun(Opts) -> smf_aaa_config:validate_handler(smf_aaa_static, Opts) end,
...
radius_handler(_Config)  ->
    ...
    ValF = fun(Opts) -> smf_aaa_config:validate_handler(smf_aaa_radius, Opts) end,
...
nasreq_handler(_Config)  ->
    Cfg = ?DIAMETER_CONFIG,
    ValF = fun(Opts) -> smf_aaa_config:validate_handler(smf_aaa_nasreq, Opts) end,
...
rf_handler(_Config)  ->
    Cfg = ?RF_CONFIG,
    ValF = fun(Opts) -> smf_aaa_config:validate_handler(smf_aaa_rf, Opts) end,
...
ro_handler(_Config)  ->
    Cfg = ?RF_CONFIG,
    ValF = fun(Opts) -> smf_aaa_config:validate_handler(smf_aaa_ro, Opts) end,
...
service(_Config)  ->
    ...
    lists:foreach(
      fun({K, V}) ->
	      Opts = smf_aaa_config:validate_handler(K, V),
	      smf_aaa:add_config(handlers, K, Opts)
      end, Functions),

    ValF = fun smf_aaa_config:validate_service/2,

    ?ok(ValF(<<"Default">>, [{handler, 'smf_aaa_static'}])),
    ?ok(ValF(<<"RADIUS-Service">>, [{handler, 'smf_aaa_radius'}])),
    ?ok(ValF(<<"DIAMETER-Service">>, [{handler, 'smf_aaa_nasreq'}])),
    ?ok(ValF(<<"RF-Service">>, maps:put(handler, 'smf_aaa_rf', ?RF_CONFIG))),
    ?ok(ValF(<<"RO-Service">>, maps:put(handler, 'smf_aaa_ro', ?RF_CONFIG))),

    ?bad(ValF(<<"RADIUS-Service">>, [{handler, 'invalid_handler'}])),
    ?bad(ValF(<<"RADIUS-Service">>, ?RADIUS_AUTH_CONFIG)),
    ?ok(ValF(<<"RADIUS-Service">>,
	     maps:put(handler, 'smf_aaa_radius', ?RADIUS_AUTH_CONFIG))),


    ?bad(ValF(<<"DIAMETER-Service">>, [{handler, 'invalid_handler'}])),
    ?bad(ValF(<<"DIAMETER-Service">>, ?DIAMETER_CONFIG)),
    ?ok(ValF(<<"DIAMETER-Service">>,
	     maps:put(handler, 'smf_aaa_nasreq', ?DIAMETER_CONFIG))),

    %% make sure the handler config is passed through to the service
    ?match_map(?STATIC_CONFIG, ValF(<<"Default">>, [{handler, 'smf_aaa_static'}])),
    ?match_map(?DIAMETER_CONFIG, ValF(<<"DIAMETER-Service">>, [{handler, 'smf_aaa_nasreq'}])),
    ?match_map(?RADIUS_ACCT_CONFIG, ValF(<<"RADIUS-Service">>, [{handler, 'smf_aaa_radius'}])),
    ok.
...
app(_Config)  ->
	...
	[{<<"Default">>,          #{handler => 'smf_aaa_static'}},
	 {<<"RADIUS-Service">>,   #{handler => 'smf_aaa_radius'}},
	 {<<"DIAMETER-Service">>, #{handler => 'smf_aaa_nasreq'}},
	 {<<"RF-Service">>,       maps:put(handler, 'smf_aaa_rf', ?RF_CONFIG)},
	 {<<"RO-Service">>,       maps:put(handler, 'smf_aaa_ro', ?RF_CONFIG)}],
    lists:foreach(
      fun({K, V}) ->
	      Opts = smf_aaa_config:validate_service(K, V),
	      smf_aaa:add_config(services, K, Opts)
      end, Services),
    ...
    ValF = fun smf_aaa_config:validate_app/2,
