...
check_stats_per_testcase() ->
    case get_session_stats(smf_aaa_radius, started) of
	0 -> ok;
	Other -> ct:fail(Other)
    end.
...
avp_filter(Config) ->
    ...
    Opts = smf_aaa_radius:validate_procedure(default, all, all,
...
set_service_pars(NewOpts) ->
    {ok, Apps0} = application:get_env(smf_aaa, apps),
    ...
    application:set_env(smf_aaa, apps, Apps),
    Apps0.
