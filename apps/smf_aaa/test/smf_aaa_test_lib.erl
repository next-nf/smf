...
-export([clear_app_env/0, smf_aaa_init/1]).

-include("smf_aaa_test_lib.hrl").
-include("../src/smf_aaa_internal.hrl").
...
clear_app_env() ->
    [application:unset_env(smf_aaa, Par) || {Par, _} <- application:get_all_env(smf_aaa)].
...
smf_aaa_init(rate_limits, #{rate_limits := Limits}) when is_list(Limits) ->
    lists:foreach(smf_aaa:setopt(rate_limit, _), Limits);
smf_aaa_init(rate_limits, #{rate_limits := Limits}) when is_map(Limits) ->
...
outstanding_reqs() ->
    lists:foldl(
      fun({_, O, _, _, _, _, _, _}, S) -> S + O end, 0, smf_aaa_diameter_srv:peers()).
