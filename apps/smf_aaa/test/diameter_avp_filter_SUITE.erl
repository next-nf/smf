...
do_filter_test(Filter, SessionMod) ->
    ...
    meck:expect(smf_aaa_ro, invoke, MECK),
    Session = init_session(SessionMod),
    GyOpts = #{credits => #{1000 => empty}},
    AAA0 = smf_aaa_session:new(Session),
    DiameterSId1 = maps:get('Diameter-Session-Id',
			     smf_aaa_session:get_session(AAA0)),
    set_diameter_session_handler(DiameterSId1, DTRA),
    {ok, _AAA1, _} = smf_aaa_session:call(AAA0, GyOpts,
					    {gy, 'CCR-Initial'}, []),
    Before = (?GET_TC_INFO(received)),
    AAA2 = smf_aaa_session:new(Session),
    DiameterSId2 = maps:get('Diameter-Session-Id',
			     smf_aaa_session:get_session(AAA2)),
    set_avp_filter(DiameterSId2, Filter),
    set_diameter_session_handler(DiameterSId2, DTRA),
    {ok, _AAA3, _} = smf_aaa_session:call(AAA2, GyOpts,
					    {gy, 'CCR-Initial'}, []),
