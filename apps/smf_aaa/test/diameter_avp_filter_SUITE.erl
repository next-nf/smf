...
do_filter_test(Filter, SessionMod) ->
    ...
    {ok, SId1} = smf_aaa_session_sup:new_session(self(),
						  Session),
    {ok, DiameterSId1} = smf_aaa_session:get(SId1,
					      'Diameter-Session-Id'),
    set_diameter_session_handler(DiameterSId1, DTRA),
    {ok, _, _} = smf_aaa_session:invoke(SId1, GyOpts,
					 {gy, 'CCR-Initial'}, []),
    Before = (?GET_TC_INFO(received)),
    {ok, SId2} = smf_aaa_session_sup:new_session(self(),
						  Session),
    {ok, DiameterSId2} = smf_aaa_session:get(SId2,
					      'Diameter-Session-Id'),
    set_avp_filter(DiameterSId2, Filter),
    set_diameter_session_handler(DiameterSId2, DTRA),
    {ok, _, _} = smf_aaa_session:invoke(SId2, GyOpts,
					 {gy, 'CCR-Initial'}, []),
