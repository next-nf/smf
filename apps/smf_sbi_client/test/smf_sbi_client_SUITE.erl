...
init_per_suite(Config) ->
    ...
    _ = smf_sbi_client_config:validate_options(fun smf_sbi_client_config:validate_option/2, TestCfg),
    ok = smf_sbi_client:setup(TestCfg),
    Config.
...
%%--------------------------------------------------------------------
session() ->
    %% copied from smf_core/pgw_SUITE
...
from_session(_Config) ->
    Req = smf_sbi_client:from_session(session()),
    ct:pal("Req: ~p~n", [Req]),
...
upf_selection(_Config) ->
    ...
    ?match({ok, {_,_,_,_}},
	   smf_sbi_client:upf_selection(Session#{'3GPP-MSISDN' => <<"440000000000">>})),
    ?match({ok, {_,_,_,_,_,_,_,_}},
	   smf_sbi_client:upf_selection(Session#{'3GPP-MSISDN' => <<"440000000001">>})),
    ?match({ok, F} when is_binary(F),
	   smf_sbi_client:upf_selection(Session#{'3GPP-MSISDN' => <<"440000000002">>})),
    ?match({error, _},
	   smf_sbi_client:upf_selection(Session#{'3GPP-MSISDN' => <<"440000000009">>})),
    ok.
