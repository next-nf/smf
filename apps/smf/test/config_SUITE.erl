...
avp_filters(Config)  ->
    ...
    ?match(#{aaa := #{handlers := #{smf_aaa_nasreq := #{avp_filter := [_|_]}}}}, Cfg),
    ct:pal("Cfg: ~p", [Cfg]),
    #{aaa := #{handlers := #{smf_aaa_nasreq := #{avp_filter := Filters}}}} = Cfg,
...
load(Dir, File, Type) ->
    ...
    ct:pal("Cfg: ~p~n", [Cfg]),
    application:set_env(smf, config, Cfg),

    Config = smf_config:load(),
...
http_api(_Config) ->
    API = [{enabled, false}],
    ValF = fun smf_http_api:validate_options/1,
...
read_json(Dir, File) ->
    ...
    smf_config:coerce_config(Config).

set(Keys, Value, Config) ->
    smf_config:set(Keys, Value, Config).
