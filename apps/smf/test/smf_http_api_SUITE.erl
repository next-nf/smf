...
init_per_suite(Config0) ->
    ...
    application:load(smf),
    CfgSet = #{type => json, file => filename:join(Dir, "ggsn.json")},
    application:set_env(smf, config, CfgSet),
...
end_per_suite(Config) ->
    ...
    [application:stop(App) || App <- [smf_core, smf_aaa, smf_cluster, smf]],
    inets:stop(),
    ok.
...
http_api_version_req(_Config) ->
    ...
    {ok, {_, _, Body}} = httpc:request(get, {URL, []},
				       [], [{body_format, binary}]),
    {ok, Vsn} = application:get_key(smf_core, vsn),
...
http_api_status_req(_Config) ->
    SysInfo = maps:from_list(smf_core:system_info()),
...
http_api_status_accept_new_get_req(_Config) ->
    ...
    {ok, {_, _, Body}} = httpc:request(get, {URL, []},
				       [], [{body_format, binary}]),
    Response = jsx:decode(Body, [return_maps]),
    AcceptNew = smf_core:system_info(accept_new),
...
http_api_status_accept_new_post_req(_Config) ->
    ...
    ?equal(smf_core:system_info(accept_new), Result1),
    ?equal(false, Result1),
    ...
    ?equal(smf_core:system_info(accept_new), Result2),
    ?equal(true, Result2),
...
http_api_prometheus_metrics_req(_Config) ->
    ...
    Result =
	lists:filter(fun(<<"smf_local_pool_free", _/binary>>) ->
			     true;
			(_) -> false
		     end, Lines),
...
http_api_delete_sessions(Config) ->
    ...
    ContextsCount = length(smf_api:contexts(all)),
    ...
    ?match(0, length(smf_api:contexts(all))),
...
get_test_url(Path) ->
    Port = ranch:get_port(smf_http_listener),
