...
-include_lib("smf_core/include/smf.hrl").
...
lib_init_per_group(Config0) ->
    ...
    [application:load(App) || App <- [cowboy, smf, smf_core, smf_aaa]],
    meck_init(Config),
    ...
    clear_app_env(),
    application:set_env(smf, config, #{type => json, file => filename:join(Dir, File)}),
    {ok, Started} = application:ensure_all_started(smf),
    ct:pal("Started: ~p", [Started]),

    smf:wait_till_running(),

    Config.

lib_end_per_group(Config) ->
    [application:stop(App) || App <- [ranch, cowboy, smf_core, smf_aaa, smf_cluster, smf]],
    meck_unload(Config),
    ok = smf_test_sx_up:stop('pgw-u01'),
    ok = smf_test_sx_up:stop('pgw-u02'),
    ok = smf_test_sx_up:stop('sgw-u'),
...
clear_app_env() ->
	...
	App <- [smf_core, smf_aaa, smf_cluster]].
...
load_aaa_answer_config(AnswerCfg) ->
    ...
    UpdCfg = smf_core_config:to_map(Answers),
