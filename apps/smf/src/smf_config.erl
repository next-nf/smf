...
load_config() ->
    {ok, Config} = application:get_env(smf, config),
    load_config(Config).
...
load_static_config() ->
    ...
    maps:from_list([{K, V} || K <- Static,
			      {ok, V} <- [application:get_env(smf_core, K)]]).
...
smf_aaa_init(product_name, #{product_name := PN0}) ->
    PN = smf_aaa_config:validate_option(product_name, to_binary(PN0)),
...
smf_sbi_client_init(Opts) ->
    smf_sbi_client_config:validate_options(fun smf_sbi_client_config:validate_option/2, Opts).
...
smf_core_init(aaa, #{aaa := AAA}) ->
    smf_aaa_init(AAA);
smf_core_init(sbi_client, #{sbi_client := SbiClient}) ->
    smf_sbi_client_init(SbiClient);
smf_core_init(wait_till_running, _) ->
    smf_core:wait_till_running();
smf_core_init(path_management, #{path_management := PathM}) ->
    ...
    ok = smf_core:setopts(path_management, Config);
smf_core_init(gtp_peers, #{gtp_peers := Peers}) ->
    maps:map(fun smf_core:set_gtp_peer_opts/2, Peers);
...
smf_core_init(upf_nodes, #{upf_nodes := UP}) ->
    ...
    maps:map(fun smf_core:add_sx_node/2, maps:get(entries, UP, #{}));
...
smf_core_init(http_api, #{http_api := Opts}) ->
    smf_http_api:init(Opts);
smf_core_init(_K, _) ->
    ok.
...
load_schemas() ->
    jesse:load_schemas(
      filename:join([code:lib_dir(smf), "priv", "schemas"]),
      fun (Bin) ->
...
delegate_aaa_handler(Map) when is_map(Map) ->
    Meta =
	case to_atom(get_key(handler, Map)) of
	    smf_aaa_gx     -> smf_meta_aaa_handler_gx();
	    smf_aaa_nasreq -> smf_meta_aaa_handler_nasreq();
	    smf_aaa_radius -> smf_meta_aaa_handler_radius();
	    smf_aaa_rf     -> smf_meta_aaa_handler_rf();
	    smf_aaa_ro     -> smf_meta_aaa_handler_ro();
	    smf_aaa_static -> smf_meta_aaa_handler_static()
	end,
    normalize_meta(Meta).

smf_meta_aaa_handler_gx() ->
...
smf_meta_aaa_handler_radius() ->
    #{handler => module,
      server => smf_meta_radius_server(),
      server_name => binary,
      client_name => binary,
      timeout => integer,
      retries => integer,
...
smf_meta_radius_server() ->
    #{host => host_or_ip,
      port => integer,
      secret => binary}.

smf_meta_aaa_handler_rf() ->
    #{handler => module,
      function => binary,
      accounting => atom,
      'Destination-Host' => binary,
...
validate_config_with_schema(Config) ->
    Schema = filename:join([code:lib_dir(smf), "priv", "schemas", "smf_config.yaml"]),
...
to_aaa_avp(K, V)
  when is_binary(V) andalso K =:= 'Tariff-Time' ->
...
from_ip(IP) when is_binary(IP) ->
    from_ip_address(smf_inet:bin2ip(IP)).
...
from_ip46(IP) when is_binary(IP) ->
    from_ip46_address(smf_inet:bin2ip(IP)).
