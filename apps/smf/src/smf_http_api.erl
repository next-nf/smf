...
init(Opts0) ->
    ...
    %% HTTP API options should be already validated in the smf_core_config,
...
start_http_listener(#{enabled := true} = Opts) ->
    Dispatch = cowboy_router:compile(
		 [{'_',
		   [
		    ...
		    {"/api/v1/spec/ui/[...]", cowboy_static, {priv_dir, smf_core, "static"}}]}
		 ]),
    ...
    cowboy:start_clear(smf_http_listener, TransOpts, ProtoOpts);
...
validate_options(Opts) when is_map(Opts) ->
    smf_core_config:mandatory_keys([enabled], Opts),
    smf_core_config:validate_options(fun validate_option/2, Opts, ?Defaults);
