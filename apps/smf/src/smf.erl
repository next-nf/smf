...
init(Config) ->
    ...
    {_, Mref} = erlang:spawn_monitor(
		  fun() -> exit(smf_cluster:wait_till_ready()) end),
...
handle_event(info, {'DOWN', Mref, _, _, ok}, startup,
    ...
    ?LOG(info, "smf: starting cluster, runtime system started in ~w ms",
    ...
    {_, ClusterMref} = erlang:spawn_monitor(
		  fun() -> exit(smf_cluster:start(Cluster)) end),
    {next_state, ready, Data#{startup => ClusterMref}};

handle_event(info, {'DOWN', Mref, _, _, Reason}, startup, #{startup := Mref}) ->
    ?LOG(critical, "smf: support applications failed to start with ~0p", [Reason]),
    {stop, {shutdown, Reason}};

handle_event(info, {'DOWN', Mref, _, _, ok}, ready,
	     #{init := Now, startup := Mref, config := Config} = Data) ->
    smf_config:apply(Config),

    ?LOG(info, "smf: ready to process requests, system started in ~w ms",
	 [erlang:convert_time_unit(erlang:monotonic_time() - Now, native, millisecond)]),
