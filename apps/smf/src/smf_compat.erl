...
get_json() ->
    ...
    AAA = setup:get_all_env(smf_aaa),
    A1 = aaa_translate_config(AAA),

    C2 = C1#{node => NodeOpts, aaa => A1},
    smf_config:serialize_config(C2).
...
translate_option(cluster, Value) when ?is_opts(Value) ->
    smf_cluster_translate_options(Value);
translate_option(sockets, Value) when ?is_opts(Value) ->
    smf_socket_translate_options(Value);
...
translate_option(http_api, Value) when ?is_opts(Value) ->
    smf_http_api_translate_options(Value);
translate_option(charging, Opts)
  when ?non_empty_opts(Opts) ->
    check_unique_keys(charging, Opts),
    C0 = translate_options(fun smf_charging_translate_options/1, Opts, [], map),
    {P, R, RB} =
	maps:fold(
...
translate_option(teid, Value) ->
    smf_tei_mngr_translate_option(Value);
translate_option(metrics, Opts) ->
    smf_prometheus_translate_options(Opts);
...
translate_vrf_to_ip_pool({Pool, Opts0}) ->
    ...
    {<<"pool-", (to_binary(Pool))/binary>>, Opts#{handler => smf_local_pool}}.
...
translate_node_selection_option(Key, {static, Opts})
  when is_atom(Key), is_list(Opts), length(Opts) > 0 ->
    Entries = lists:map(fun smf_node_selection_translate_static_option/1, Opts),
    #{type => static, entries => Entries};
translate_node_selection_option(Key, {dns, Opts})
  when is_atom(Key) ->
    smf_node_selection_translate_dns(Opts);
...
translate_node_default_option({ip_pools, Pools})
  when is_list(Pools) ->
    V = [smf_ip_pool_translate_name(ip_pools, Name) || Name <- Pools],
...
translate_ip_pools({Name, Values})
  when ?is_opts(Values) ->
    {smf_ip_pool_translate_name(ip_pools, Name), smf_ip_pool_translate_options(Values)};
...
translate_apn_option({ip_pools = Opt, Pools})
  when is_list(Pools) ->
    V = [smf_ip_pool_translate_name(Opt, Name) || Name <- Pools],
    check_unique_elements(Opt, V),
    {Opt, V};
...
smf_cluster_translate_options(Values) ->
    translate_options(fun smf_cluster_translate_option/2, Values, ?ClusterDefaults, map).

smf_cluster_translate_option(enabled, Value) when is_boolean(Value) ->
    Value;
smf_cluster_translate_option(initial_timeout, Value)
  when is_integer(Value), Value > 0 ->
    Value;
smf_cluster_translate_option(release_cursor_every, Value)
  when is_integer(Value) ->
    Value;
smf_cluster_translate_option(seed_nodes, {M, F, A} = Value)
  when is_atom(M), is_atom(F), is_list(A) ->
    Value;
smf_cluster_translate_option(seed_nodes, Nodes) when is_list(Nodes) ->
    lists:foreach(
    ...
    Nodes;
smf_cluster_translate_option(Opt, Value) ->
    throw({error, {options, {Opt, Value}}}).

smf_socket_translate_options(Values) when is_list(Values), length(Values) >= 1 ->
    check_unique_keys(sockets, Values),
    translate_options(fun smf_socket_translate_option/2, Values, [], map);
smf_socket_translate_options(Values) ->
    throw({error, {options, {sockets, Values}}}).

smf_socket_translate_option(Name, Values)
  when is_atom(Name), ?is_opts(Values) ->
    case get_opt(type, Values, undefined) of
	'gtp-c' ->
	    smf_gtp_socket_translate_options(Name, Values);
	'gtp-u' ->
	    smf_gtp_socket_translate_options(Name, Values);
	'pfcp' ->
	    smf_sx_socket_translate_options(Name, Values);
	_ ->
	    throw({error, {options, {Name, Values}}})
    end;
smf_socket_translate_option(Opt, Values) ->
    throw({error, {options, {Opt, Values}}}).
...
smf_gtp_socket_translate_options(Name, Values) ->
    translate_options(
      fun smf_gtp_socket_translate_option/2, Values, [{name, Name}|?GtpSocketDefaults], map).

smf_gtp_socket_translate_option(name, Value) when is_atom(Value) ->
    Value;
smf_gtp_socket_translate_option(type, 'gtp-c' = Value) ->
    Value;
smf_gtp_socket_translate_option(type, 'gtp-u' = Value) ->
    Value;
smf_gtp_socket_translate_option(ip, Value)
  when is_tuple(Value) andalso
       (tuple_size(Value) == 4 orelse tuple_size(Value) == 8) ->
    Value;
smf_gtp_socket_translate_option(cluster_ip, Value)
  when is_tuple(Value) andalso
       (tuple_size(Value) == 4 orelse tuple_size(Value) == 8) ->
    Value;
smf_gtp_socket_translate_option(netdev, Value) when is_list(Value) ->
    Value;
smf_gtp_socket_translate_option(netdev, Value) when is_binary(Value) ->
    unicode:characters_to_list(Value, latin1);
smf_gtp_socket_translate_option(netns, Value) when is_list(Value) ->
    Value;
smf_gtp_socket_translate_option(netns, Value) when is_binary(Value) ->
    unicode:characters_to_list(Value, latin1);
smf_gtp_socket_translate_option(vrf, Value) ->
    vrf_translate_name(Value);
smf_gtp_socket_translate_option(freebind, Value) when is_boolean(Value) ->
    Value;
smf_gtp_socket_translate_option(reuseaddr, Value) when is_boolean(Value) ->
    Value;
smf_gtp_socket_translate_option(send_port, Port)
  when is_integer(Port) andalso
       (Port =:= 0 orelse (Port >= 1024 andalso Port < 65536)) ->
    Port;
smf_gtp_socket_translate_option(send_port, true) ->
    0;
smf_gtp_socket_translate_option(send_port, false) ->
    false;
smf_gtp_socket_translate_option(rcvbuf, Value)
  when is_integer(Value) andalso Value > 0 ->
    Value;
smf_gtp_socket_translate_option(burst_size, Value)
  when is_integer(Value) andalso Value > 0 ->
    Value;
smf_gtp_socket_translate_option(Opt, Value) ->
    throw({error, {options, {Opt, Value}}}).
...
smf_sx_socket_translate_options(Name, Values) ->
    Opts = translate_options(fun smf_sx_socket_translate_option/2,
			     Values, [{name, Name}|?SxSocketDefaults], map),
    maps:without([node], Opts).

smf_sx_socket_translate_option(type, pfcp = Value) ->
    Value;
smf_sx_socket_translate_option(node, Value) ->
    Value;
smf_sx_socket_translate_option(name, Value) when is_atom(Value) ->
    Value;
smf_sx_socket_translate_option(ip, Value)
  when is_tuple(Value) andalso
       (tuple_size(Value) == 4 orelse tuple_size(Value) == 8) ->
    Value;
smf_sx_socket_translate_option(netdev, Value) when is_list(Value) ->
    Value;
smf_sx_socket_translate_option(netdev, Value) when is_binary(Value) ->
    unicode:characters_to_list(Value, latin1);
smf_sx_socket_translate_option(netns, Value) when is_list(Value) ->
    Value;
smf_sx_socket_translate_option(netns, Value) when is_binary(Value) ->
    unicode:characters_to_list(Value, latin1);
smf_sx_socket_translate_option(freebind, Value) when is_boolean(Value) ->
    Value;
smf_sx_socket_translate_option(reuseaddr, Value) when is_boolean(Value) ->
    Value;
smf_sx_socket_translate_option(rcvbuf, Value)
  when is_integer(Value) andalso Value > 0 ->
    Value;
smf_sx_socket_translate_option(socket, Value)
  when is_atom(Value) ->
    Value;
smf_sx_socket_translate_option(burst_size, Value)
  when is_integer(Value) andalso Value > 0 ->
    Value;
smf_sx_socket_translate_option(Opt, Value) ->
    throw({error, {options, {Opt, Value}}}).
...
smf_http_api_translate_options(Values) ->
    translate_options(fun smf_http_api_translate_option/1, Values, ?HttpDefaults, map).

smf_http_api_translate_option({port, Port} = Opt)
  when is_integer(Port), Port >= 0, Port =< 65535 ->
    Opt;
smf_http_api_translate_option({acceptors_num, Acceptors})
  when is_integer(Acceptors) ->
    {num_acceptors, Acceptors};
smf_http_api_translate_option({num_acceptors, Acceptors} = Opt)
  when is_integer(Acceptors) ->
    Opt;
smf_http_api_translate_option({ip, Value} = Opt)
  when is_tuple(Value) andalso
       (tuple_size(Value) == 4 orelse tuple_size(Value) == 8) ->
    Opt;
smf_http_api_translate_option({ipv6_v6only, Value} = Opt) when is_boolean(Value) ->
    Opt;
smf_http_api_translate_option(Opt) ->
...
smf_charging_translate_options({Key, Opts})
  when is_atom(Key), ?is_opts(Opts) ->
    {Key, translate_options(
	    fun smf_charging_translate_charging_options/2, Opts, ?DefaultChargingOpts, map)}.

%% smf_charging_translate_rule_def('Service-Identifier', Value) ->
%% smf_charging_translate_rule_def('Rating-Group', Value) ->
%% smf_charging_translate_rule_def('Flow-Information', Value) ->
%% smf_charging_translate_rule_def('Default-Bearer-Indication', Value) ->
%% smf_charging_translate_rule_def('TDF-Application-Identifier', Value) ->
%% smf_charging_translate_rule_def('Flow-Status', Value) ->
%% smf_charging_translate_rule_def('QoS-Information', Value) ->
%% smf_charging_translate_rule_def('PS-to-CS-Session-Continuity', Value) ->
%% smf_charging_translate_rule_def('Reporting-Level', Value) ->
%% smf_charging_translate_rule_def('Online', Value) ->
%% smf_charging_translate_rule_def('Offline', Value) ->
%% smf_charging_translate_rule_def('Max-PLR-DL', Value) ->
%% smf_charging_translate_rule_def('Max-PLR-UL', Value) ->
%% smf_charging_translate_rule_def('Metering-Method', Value) ->
%% smf_charging_translate_rule_def('Precedence', Value) ->
%% smf_charging_translate_rule_def('AF-Charging-Identifier', Value) ->
%% smf_charging_translate_rule_def('Flows', Value) ->
%% smf_charging_translate_rule_def('Monitoring-Key', Value) ->
%% smf_charging_translate_rule_def('Redirect-Information', Value) ->
%% smf_charging_translate_rule_def('Mute-Notification', Value) ->
%% smf_charging_translate_rule_def('AF-Signalling-Protocol', Value) ->
%% smf_charging_translate_rule_def('Sponsor-Identity', Value) ->
%% smf_charging_translate_rule_def('Application-Service-Provider-Identity', Value) ->
%% smf_charging_translate_rule_def('Required-Access-Info', Value) ->
%% smf_charging_translate_rule_def('Sharing-Key-DL', Value) ->
%% smf_charging_translate_rule_def('Sharing-Key-UL', Value) ->
%% smf_charging_translate_rule_def('Traffic-Steering-Policy-Identifier-DL', Value) ->
%% smf_charging_translate_rule_def('Traffic-Steering-Policy-Identifier-UL', Value) ->
%% smf_charging_translate_rule_def('Content-Version', Value) ->

smf_charging_translate_rule_def(Key, Value)
...
smf_charging_translate_rulebase(Key, [Id | _] = RuleBaseDef)
  when is_binary(Key) andalso is_binary(Id) ->
    ...
    RuleBaseDef;
smf_charging_translate_rulebase(Key, Rule)
  when is_binary(Key) andalso ?non_empty_opts(Rule) ->
    check_unique_keys(Key, Rule),
    translate_options(fun smf_charging_translate_rule_def/2, Rule, ?DefaultRuleDef, map);
smf_charging_translate_rulebase(Key, Rule) ->
    throw({error, {options, {rulebase, {Key, Rule}}}}).

smf_charging_translate_online_charging_options(Key, Opts) ->
    throw({error, {options, {{online, charging}, {Key, Opts}}}}).

smf_charging_translate_offline_charging_triggers(Key, Opt)
...
smf_charging_translate_offline_charging_triggers(Key, Opt)
  when (Opt == 'container' orelse Opt == 'off') andalso
    ...
    Opt;
smf_charging_translate_offline_charging_triggers(Key, Opts) ->
    throw({error, {options, {{offline, charging, triggers}, {Key, Opts}}}}).

smf_charging_translate_offline_charging_options(enable, Opt) when is_boolean(Opt) ->
    Opt;
smf_charging_translate_offline_charging_options(triggers, Opts) ->
    translate_options(fun smf_charging_translate_offline_charging_triggers/2,
		     Opts, ?DefaultOfflineChargingTriggers, map);
smf_charging_translate_offline_charging_options(Key, Opts) ->
    throw({error, {options, {{offline, charging}, {Key, Opts}}}}).


smf_charging_translate_charging_options(rulebase, RuleBase) ->
    check_unique_keys(rulebase, RuleBase),
    translate_options(fun smf_charging_translate_rulebase/2, RuleBase, ?DefaultRulebase, map);
smf_charging_translate_charging_options(online, Opts) ->
    translate_options(fun smf_charging_translate_online_charging_options/2,
		     Opts, ?DefaultOnlineChargingOpts, map);
smf_charging_translate_charging_options(offline, Opts) ->
    translate_options(fun smf_charging_translate_offline_charging_options/2,
		     Opts, ?DefaultOfflineChargingOpts, map);
smf_charging_translate_charging_options(Key, Opts) ->
...
smf_tei_mngr_translate_option({Prefix, Len} = Value)
...
smf_prometheus_translate_options(Opts) ->
    translate_options(fun smf_prometheus_translate_option/2, Opts, ?DefaultMetricsOpts, map).

smf_prometheus_translate_option(gtp_path_rtt_millisecond_intervals = Opt, Value) ->
...
smf_node_selection_translate_ip_list(L) ->
    lists:map(
      fun(IP) when ?IS_IP(IP) -> IP;
	 (IP) -> throw({error, {options, {ip, IP}}})
      end, L).

smf_node_selection_translate_static_option({Label, {Order, Prio}, [{_,_}|_] = Services, Host})
  when is_list(Label),
       is_integer(Order),
       is_integer(Prio),
       is_list(Host) ->
...
smf_node_selection_translate_static_option({Host, IP4, IP6})
  when is_list(Host),
       is_list(IP4),
       is_list(IP6),
    ...
    #{type => host,
      name => to_binary(Host),
      ip4 => smf_node_selection_translate_ip_list(IP4),
      ip6 => smf_node_selection_translate_ip_list(IP6)};
smf_node_selection_translate_static_option(Opt) ->
    throw({error, {options, {static, Opt}}}).

smf_node_selection_translate_dns(IP) when ?IS_IP(IP) ->
    #{type => dns, server => IP, port => 53};
smf_node_selection_translate_dns({IP, Port})
...
smf_ip_pool_translate_options(Options) ->
    case get_opt(handler, Options, smf_local_pool) of
	smf_local_pool ->
	    smf_local_pool_translate_options(Options);
	Handler ->
	    throw({error, {options, {handler, Handler}}})
    end.

smf_ip_pool_translate_name(_, Name) when is_binary(Name) ->
    Name;
smf_ip_pool_translate_name(_, Name) when is_list(Name) ->
    unicode:characters_to_binary(Name, utf8);
smf_ip_pool_translate_name(_, Name) when is_atom(Name) ->
    atom_to_binary(Name, utf8);
smf_ip_pool_translate_name(Opt, Name) ->
   throw({error, {options, {Opt, Name}}}).

-define(DefaultPoolOptions, [{handler, smf_local_pool}, {ranges, []}]).

smf_local_pool_translate_options(Options) ->
    translate_options(fun smf_local_pool_translate_option/2, Options, ?DefaultPoolOptions, map).

smf_local_pool_translate_ip_range({Start, End, PrefixLen} = Range)
...
smf_local_pool_translate_ip_range(Range) ->
    throw({error, {options, {range, Range}}}).

smf_local_pool_translate_option(ranges, Ranges)
  when is_list(Ranges), length(Ranges) /= 0 ->
    [begin
	 {Start, End, Len} = smf_local_pool_translate_ip_range(X),
	 #{start => Start, 'end' => End, prefix_len => Len}
     end || X <- Ranges];
smf_local_pool_translate_option(Opt, Value)
  when Opt == 'MS-Primary-DNS-Server';   Opt == 'MS-Secondary-DNS-Server';
       Opt == 'MS-Primary-NBNS-Server';  Opt == 'MS-Secondary-NBNS-Server';
       Opt == 'DNS-Server-IPv6-Address'; Opt == '3GPP-IPv6-DNS-Servers' ->
    translate_ip_cfg_opt(Opt, Value);
smf_local_pool_translate_option(Opt, Pool)
  when Opt =:= 'Framed-Pool';
       Opt =:= 'Framed-IPv6-Pool' ->
    smf_ip_pool_translate_name(Opt, Pool);
smf_local_pool_translate_option(handler, Value) ->
    Value;
smf_local_pool_translate_option(Opt, Value) ->
    throw({error, {options, {Opt, Value}}}).
...
-define(DefaultAAAOpts,
	#{
	  'AAA-Application-Id' => smf_aaa_provider,
	  'Username' => #{default => <<"smf">>,
			  from_protocol_opts => true},
	  'Password' => #{default => <<"smf">>}
	 }).
...
aaa_translate_handler(smf_aaa_static, Opts0) when is_map(Opts0) ->
    {Answers, Opts} = take_opt(answers, Opts0, #{}),
    #{answers => aaa_translate_answers(Answers), defaults => Opts};
aaa_translate_handler(smf_aaa_radius, Opts) ->
