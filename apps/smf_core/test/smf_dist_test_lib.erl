...
node_init_per_group(Id, Config) ->
    ...
    smf_test_lib:load_config(per_node_config(Id, AppCfg)),
...
node_init_apps_per_group(Id, Config) ->
    {_, AppCfg} = lists:keyfind(app_cfg, 1, Config),   %% let it crash if undefined
    smf_test_lib:init_apps(per_node_config(Id, AppCfg)),
...
init_per_group(Config) ->
    ...
    MasterCfg = smf_test_lib:init_ets(Config),
...
-define(NODE_OPTS, [{[kernel, logger, '_', config, file], log},
		    {[smf_core, node, node_id], node_id},
		    {[smf_core, sockets, '_', ip], ip}
		   ]).
...
match_dist_metric(Type, Name, LabelValues, Cond, Expected, File, Line, Cnt, Config) ->
    Nodes = proplists:get_value(nodes, Config),
    NodeNames = [N || {_, _, N} <- Nodes],
    Res = erpc:multicall(NodeNames, smf_dist_test_lib, match_metric,
...
wait4contexts(Cnt) ->
    case smf_test_lib:active_contexts() of
	0 -> ok;
	Actual ->
	    if Cnt > 100 ->
		    timer:sleep(10),
