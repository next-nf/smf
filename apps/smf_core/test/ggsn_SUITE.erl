...
cache_timeout(Config) ->
    Socket = smf_socket_reg:lookup('gtp-c', 'irx'),
    {GtpC, _, _} = create_pdp_context(Config),
    delete_pdp_context(GtpC),
    ...
    {T0, Q0} = smf_gtp_c_socket:get_response_q(Socket),
    ?match(X when X /= 0, length(T0)),
    ?match(X when X /= 0, length(Q0)),

    ct:sleep({seconds, 120}),

    {T1, Q1} = smf_gtp_c_socket:get_response_q(Socket),
    ?equal(0, length(T1)),
    ?equal(0, length(Q1)),
...
sx_ondemand(Config) ->
    ?equal(1, maps:size(smf_sx_node_reg:available())),

    {GtpC, _, _} = create_pdp_context(async_sx, Config),
    delete_pdp_context(GtpC),

    ?equal(2, maps:size(smf_sx_node_reg:available())),
    ?equal([], outstanding_requests()),
    ok = meck:wait(?HUT, terminate, '_', ?TIMEOUT).
...
simple_ofcs(Config) ->
	...
	#{volume_measurement =>
	  ...
	  time_of_first_packet =>
	      #time_of_first_packet{time = smf_gsn_lib:seconds_to_sntp_time(StartTS + 24)},
	  time_of_last_packet =>
	      #time_of_last_packet{time = smf_gsn_lib:seconds_to_sntp_time(StartTS + 180)},
...
pfcp_sx_association_metric(_Config) ->
    ?equal(ok, prometheus_registry:register_collector(smf_core, smf_prometheus_collector)),
...
load_ocs_config(Initial, Update) ->
    smf_test_lib:load_aaa_answer_config([{{gy, 'CCR-Initial'}, Initial},
