				...
				#{'Result-Code' => 2001,
				  'Charging-Rule-Install' =>
				      [#{'Charging-Rule-Definition' =>
						...
						'Flow-Information' =>
						    [#{'Flow-Description' =>
							   [<<"permit out udp from any to assigned 4000">>],
						       'Flow-Direction' => [2],
...
init_per_group(single_socket, Config0) ->
    ...
    AppCfg = set_cfg_value([smf_core, sockets, 'irx-socket', send_port], false, AppCfg0),
    Config = lists:keystore(app_cfg, 1, Config0, {app_cfg, AppCfg}),
    lib_init_per_group(Config);
init_per_group(ipv6, Config0) ->
    case smf_test_lib:has_ipv6_test_config() of
	true ->
	    Config1 = smf_core_sbi_lib:init_per_group(Config0),
...
end_per_group(Group, Config)
  when Group == ipv4; Group == ipv6 ->
    smf_core_sbi_lib:end_per_group(Config),
    ok;
end_per_group(Group, Config)
  when Group == common;
       Group == sx_fail;
...
common() ->
    [invalid_gtp_pdu,
     ...
     mme_delete_bearer_command_partial_reject,
...
setup_per_testcase(Config, ClearSxHist) ->
    ct:pal("Sockets: ~p", [smf_socket_reg:all()]),
    ct:pal("Logger: ~p", [logger:get_handler_config()]),
    smf_test_sx_up:reset('pgw-u01'),
    meck_reset(Config),
    start_gtpc_server(Config),
    reconnect_all_sx_nodes(),
    ClearSxHist andalso smf_test_sx_up:history('pgw-u01', true),
    ok.
...
init_per_testcase(create_session_request_pool_exhausted, Config) ->
    setup_per_testcase(Config),
    ok = meck:new(smf_local_pool, [passthrough, no_link]),
    Config;
...
init_per_testcase(simple_session_request_cp_teid, Config) ->
    {ok, _} = smf_test_sx_up:feature('pgw-u01', ftup, 0),
    setup_per_testcase(Config),
    Config;
init_per_testcase(simple_session_request_nf_sel, Config) ->
    smf_test_lib:set_apn_key(upf_selection, [smf_sbi_client]),
    smf_sbi_client:setup(proplists:get_value(sbi_config, Config, #{})),
    setup_per_testcase(Config),
    Config;
...
init_per_testcase(sx_connect_fail, Config) ->
    meck_reset(Config),
    start_gtpc_server(Config),
    ok = meck:new(smf_sx_node, [passthrough, no_link]),
    Config;
...
init_per_testcase(gy_validity_timer_up, Config) ->
    {ok, _} = smf_test_sx_up:feature('pgw-u01', vtime, 1),
    setup_per_testcase(Config),
    smf_test_lib:set_online_charging(true),
    smf_test_lib:load_aaa_answer_config([{{gy, 'CCR-Initial'}, 'Initial-OCS-VT'},
			    {{gy, 'CCR-Update'},  'Update-OCS-VT'}]),
...
init_per_testcase(TestCase, Config)
  when TestCase == split_charging1 ->
    setup_per_testcase(Config),
    smf_test_lib:set_online_charging(true),
    smf_test_lib:load_aaa_answer_config([{{gx, 'CCR-Initial'}, 'Initial-Gx-Split1'},
...
init_per_testcase(TestCase, Config)
  when TestCase == split_charging2 ->
    setup_per_testcase(Config),
    smf_test_lib:set_online_charging(true),
    smf_test_lib:load_aaa_answer_config([{{gx, 'CCR-Initial'}, 'Initial-Gx-Split2'},
			    {{gy, 'CCR-Initial'}, 'Initial-OCS'},
			    {{gy, 'CCR-Update'},  'Update-OCS'}]),
    Config;
init_per_testcase(aa_pool_select, Config) ->
    smf_test_sx_up:ue_ip_pools('pgw-u01', [<<"pool-A">>, <<"pool-C">>,
					    <<"pool-D">>, <<"pool-B">>]),
    setup_per_testcase(Config),
    Config;
init_per_testcase(TestCase, Config)
  when TestCase == pfcp_select;
    ...
    smf_test_sx_up:nat_port_blocks('pgw-u01', sgi, [<<"nat-A">>, <<"nat-C">>,
						     <<"nat-D">>, <<"nat-B">>]),
    smf_test_sx_up:nat_port_blocks('pgw-u01', example, [<<"nat-E">>]),
    setup_per_testcase(Config),
    Config;
init_per_testcase(TestCase, Config)
  when TestCase == tariff_time_change ->
    setup_per_testcase(Config),
    smf_test_lib:set_online_charging(true),
    smf_test_lib:load_aaa_answer_config([{{gy, 'CCR-Initial'}, 'Initial-OCS-TTC'},
...
init_per_testcase(redirect_info, Config) ->
    setup_per_testcase(Config),
    smf_test_lib:load_aaa_answer_config([{{gx, 'CCR-Initial'}, 'Initial-Gx-Redirect'}]),
    Config;
%% init_per_testcase(TestCase, Config)
...
init_per_testcase(gx_invalid_charging_rule, Config) ->
    setup_per_testcase(Config),
    smf_test_lib:load_aaa_answer_config([{{gx, 'CCR-Initial'}, 'Initial-Gx-Fail-2'}]),
    Config;
...
init_per_testcase(TC, Config)
  when TC =:= mme_delete_bearer_command_multi;
       TC =:= mme_delete_bearer_command_partial_reject ->
    setup_per_testcase(Config),
    smf_test_lib:load_aaa_answer_config([{{gx, 'CCR-Initial'}, 'Initial-Gx-BCM-UE-NW'}]),
    Config;
...
end_per_testcase(TestCase, Config)
  when TestCase == simple_ofcs;
       TestCase == ofcs_no_interim;
       TestCase == split_charging1;
       TestCase == split_charging2 ->
    ok = meck:delete(smf_aaa_charging, rf_initial, 4),
    end_per_testcase(Config);
end_per_testcase(tariff_time_change, Config) ->
    ok = meck:delete(smf_aaa_session, new_session, 1),
    ok = meck:delete(smf_aaa_charging, rf_initial, 4),
    end_per_testcase(Config);
...
end_per_testcase(create_session_request_accept_new, Config) ->
    smf_core:system_info(accept_new, true),
    end_per_testcase(Config);
...
end_per_testcase(simple_session_request_cp_teid, Config) ->
    {ok, _} = smf_test_sx_up:feature('pgw-u01', ftup, 1),
    end_per_testcase(Config);
end_per_testcase(simple_session_request_nf_sel, Config) ->
    smf_test_lib:set_apn_key(upf_selection, ['3gpp']),
    smf_sbi_client:setup(#{}),
    end_per_testcase(Config);
...
end_per_testcase(gy_validity_timer_up, Config) ->
    {ok, _} = smf_test_sx_up:feature('pgw-u01', vtime, 0),
    end_per_testcase(Config);
end_per_testcase(aa_pool_select, Config) ->
    ok = meck:delete(smf_aaa_auth, authenticate, 4),
    ok = smf_test_sx_up:ue_ip_pools('pgw-u01', [<<"pool-A">>]),
    end_per_testcase(Config);
end_per_testcase(aa_nat_select, Config) ->
    ok = meck:delete(smf_aaa_auth, authenticate, 4),
    smf_test_sx_up:nat_port_blocks('pgw-u01', sgi, []),
    smf_test_sx_up:nat_port_blocks('pgw-u01', example, []),
    end_per_testcase(Config);
end_per_testcase(pfcp_select, Config) ->
    smf_test_sx_up:nat_port_blocks('pgw-u01', sgi, []),
    smf_test_sx_up:nat_port_blocks('pgw-u01', example, []),
    end_per_testcase(Config);
%% gtp inactivity_timeout reset to default 28800000ms ~8 hrs
end_per_testcase(gtp_idle_timeout_pfcp_session_loss, Config) ->
    smf_test_lib:set_apn_key(inactivity_timeout, 28800000),
    end_per_testcase(Config);
...
create_session_request_gy_fail(Config) ->
    ...
    ?match_metric(prometheus_gauge, smf_local_pool_free, PoolId, ?IPv4PoolSize),
    ?match_metric(prometheus_gauge, smf_local_pool_used, PoolId, 0),
...
create_session_request_pool_exhausted(Config) ->
    ok = meck:expect(smf_local_pool, wait_pool_response,
    ...
    ok = meck:expect(smf_local_pool, send_pool_request,
...
create_session_request_accept_new(Config) ->
    ?equal(smf_core:system_info(accept_new, false), true),
    create_session(reject_new, Config),
    ?equal(smf_core:system_info(accept_new, true), false),
...
create_session_request_duplicate_teids(Config) ->
    ...
    ?equal(1, maps:size(smf_test_sx_up:sessions('pgw-u01'))),
    meck_validate(Config),
    ok.
...
path_restart_recovery(Config) ->
    ...
    ct:pal("ALL: ~p", [smf_api:peer(all)]),
    [?match(#{tunnels := 1}, X) || X <- smf_api:peer(all)],
...
path_restart_multi(Config) ->
    ...
    [?match(#{tunnels := 5}, X) || X <- smf_api:peer(all)],
...
path_failure(Config) ->
    ...
    ok = meck:expect(smf_gtp_c_socket, send_request,
		     fun (_, _, IP, _, _, _, #gtp{type = echo_request}, CbInfo)
			   when IP =:= ClientIP ->
			     ...
			     smf_gtp_c_socket:send_reply(CbInfo, timeout);
    ...
    [?match(#{tunnels := 0}, X) || X <- smf_api:peer(all)],
...
path_failure_suspect_timeout(Config) ->
    ok = meck:expect(gtp_path, init,
		     fun ([Parent, Socket, Version, RemoteIP, Trigger, Args0]) ->
			     ...
			     Args =
				 smf_test_lib:maps_recusive_merge(
				   Args0, #{busy =>
						#{echo => 10,
...
path_failure_suspect_echo_backoff(Config) ->
    ...
    ?match(X when X >= 7 andalso X < 15,
	   meck:num_calls(smf_gtp_c_socket, send_request, EchoMs)),
...
simple_session_request(Config) ->
    ...
    ?match_metric(prometheus_gauge, smf_local_pool_free, PoolId, ?IPv4PoolSize - 1),
    ?match_metric(prometheus_gauge, smf_local_pool_used, PoolId, 1),
    ...
    [SER|_] = lists:filter(
		fun(#pfcp{type = session_establishment_request}) -> true;
		   (_) ->false
		end, smf_test_sx_up:history('pgw-u01')),
...
simple_session_request_nf_sel(Config) ->
    ...
    SER = lists:last(
	    lists:filter(
	      fun(#pfcp{type = session_establishment_request}) -> true;
		 (_) ->false
	      end, smf_test_sx_up:history('pgw-u02'))),
...
error_indication(Config) ->
    ...
    smf_test_sx_up:send('pgw-u01', make_error_indication_report(GtpC)),
...
modify_bearer_request_ra_update(Config) ->
    ...
    ?match(UpdateCount, meck:num_calls(smf_aaa_charging, rf_update,
				       ['_', '_', '_', ContainerClosePredicate])),
    ?match(1, meck:num_calls(smf_aaa_charging, rf_terminate,
			     ['_', '_', '_', '_'])),
...
modify_bearer_request_rat_update(Config) ->
    ...
    ?match(1, meck:num_calls(smf_aaa_charging, rf_update,
			     ['_', '_', '_', CDRClosePredicate])),
    ?match(1, meck:num_calls(smf_aaa_charging, rf_terminate,
			     ['_', '_', '_', '_'])),
...
%%--------------------------------------------------------------------
delete_bearer_requests_multi() ->
    [{doc, "Check smf_api deletes multiple contexts"}].
delete_bearer_requests_multi(Config) ->
    ...
    Ref = make_ref(),
    Self = self(),
    spawn(fun() -> Self ! {req, Ref, smf_api:delete_contexts(2)} end),
...
interop_sgsn_to_sgw(Config) ->
    ...
    {GtpC1, _, _} = smf_ggsn_test_lib:create_pdp_context(Config),
...
interop_sgw_to_sgsn(Config) ->
    ...
    {GtpC1, _, _} = create_session(Config),
    ?match_metric(prometheus_gauge, gtp_path_contexts_total, CtxMetricV2, 1),
    {GtpC2, _, _} = smf_ggsn_test_lib:update_pdp_context(tei_update, GtpC1),
    ?match_metric(prometheus_gauge, gtp_path_contexts_total, CtxMetricV1, 1),
    ?match_metric(prometheus_gauge, gtp_path_contexts_total, CtxMetricV2, 0),
    smf_ggsn_test_lib:delete_pdp_context(GtpC2),
...
session_options(Config) ->
    ...
    [#{'Process' := Pid}|_] = smf_api:tunnel(all),
    #{aaa_session := Opts} = gtp_context:info(Pid),
    ct:pal("Opts: ~p", [Opts]),
...
session_accounting(Config) ->
    ...
    smf_test_sx_up:accounting('pgw-u01', off),

    SessionOpts1 = smf_test_lib:query_usage_report(PCtx),
    ?equal(false, maps:is_key('InPackets', SessionOpts1)),
    ?equal(false, maps:is_key('InOctets', SessionOpts1)),

    %% enable accouting again....
    smf_test_sx_up:accounting('pgw-u01', on),

    SessionOpts2 = smf_test_lib:query_usage_report(PCtx),
    ?match(#{'InPackets' := 3, 'OutPackets' := 1,
	     'InOctets' := 4, 'OutOctets' := 2}, SessionOpts2),

    SessionOpts3 = smf_test_lib:query_usage_report(PCtx),
    ?match(#{'InPackets' := 3, 'OutPackets' := 1,
...
sx_cp_to_up_forward(Config) ->
    ...
    SxIP = smf_inet:ip2bin(proplists:get_value(pgw_u01_sx, Config)),
    LocalIP = smf_inet:ip2bin(proplists:get_value(localhost, Config)),

    InnerGTP = gtp_packet:encode(
		 #gtp{version = v1, type = g_pdu, tei = DataTEI, ie = <<0,0,0,0,0,0,0>>}),
    InnerIP = smf_inet:make_udp(SxIP, LocalIP, ?GTP1u_PORT, ?GTP1u_PORT, InnerGTP),
    smf_test_sx_up:send('pgw-u01', InnerIP),
...
sx_upf_restart(Config) ->
    ...
    smf_test_sx_up:restart('pgw-u01'),
    ct:pal("R1: ~p", [smf_sx_node_reg:available()]),

    %% expect the first request to fail
    create_session(system_failure, Config),
    ct:pal("R2: ~p", [smf_sx_node_reg:available()]),

    wait_for_all_sx_nodes(),
    ct:pal("R3: ~p", [smf_sx_node_reg:available()]),
...
sx_timeout(Config) ->
    ...
    ok = meck:expect(smf_sx_socket, call,
...
sx_connect_fail(Config) ->
    ...
    Self = self(),
    SxNodes = supervisor:which_children(smf_sx_node_sup),
    %%?match([_], SxNodes),
    ct:pal("SxNodes: ~p", [SxNodes]),
    Expect =
	[begin
	     Ref = make_ref(),
	     smf_sx_node:test_cmd(Pid, reconnect),
	     smf_sx_node:notify_up(Pid, {Self, Ref}),
	     Ref
    ...
    meck_validate(Config),
    true = meck:validate(smf_sx_node),
...
sx_ondemand(Config) ->
    SxNodeAvail = maps:size(smf_sx_node_reg:available()),

    {GtpC, _, _} = create_session(async_sx, Config),
    delete_session(GtpC),

    ?equal(SxNodeAvail + 1, maps:size(smf_sx_node_reg:available())),
    ?equal([], outstanding_requests()),
...
pfcp_session_deleted_by_the_up_function(Config) ->
    ...
    lists:foreach(
      fun(_X) ->
	      ct:sleep(100),
	      smf_test_sx_up:usage_report('pgw-u01', PCtx, MatchSpec, IEs, Report)
      end, lists:seq(1, 3)),

    ct:sleep(100),
    delete_session(not_found, GtpC),

    ?equal(true, meck:called(smf_aaa_pcf, ccr_terminate, ['_', '_', '_', '_'])),
    ?equal(true, meck:called(smf_aaa_charging, gy_ccr_terminate, ['_', '_', '_', '_'])),
    ?equal(true, meck:called(smf_aaa_auth, stop, ['_', '_', '_', '_'])),
...
gy_validity_timer_up(Config) ->
    ...
    lists:foreach(
      fun(_X) ->
	      ct:sleep(100),
	      smf_test_sx_up:usage_report('pgw-u01', PCtx, MatchSpec, Report)
      end, lists:seq(1, 3)),
...
simple_aaa(Config) ->
    ...
    H = meck:history(smf_aaa_auth),
    SInv =
	lists:filter(
	  fun({_, {smf_aaa_auth, Procedure, _}, _})
...
simple_ofcs(Config) ->
    ...
    Report =
	#{volume_measurement => #volume_measurement{total = 5, uplink = 2, downlink = 3},
	  time_of_first_packet => #time_of_first_packet{time = smf_gsn_lib:seconds_to_sntp_time(StartTS + 24)},
	  time_of_last_packet => #time_of_last_packet{time = smf_gsn_lib:seconds_to_sntp_time(StartTS + 180)},
	  start_time => #start_time{time = smf_gsn_lib:seconds_to_sntp_time(StartTS)},
	  end_time => #end_time{time = smf_gsn_lib:seconds_to_sntp_time(StartTS + 600)},
	  tp_packet_measurement => #tp_packet_measurement{total = 12, uplink = 5, downlink = 7}},
    ReportFun =
	fun({Id, Type}, Reports) ->
		Trigger =
    ...
    smf_test_sx_up:usage_report('pgw-u01', PCtx, MatchSpec, ReportFun),
...
ofcs_no_interim(Config) ->
    ...
    ?equal(true, smf_context:test_cmd(gtp, CtxKey, is_alive)),
    {ok, _} = smf_context:test_cmd(gtp, CtxKey, pfcp_ctx),
...
secondary_rat_usage_data_report(Config) ->
    ...
    H = meck:history(smf_aaa_charging),
    SInv =
	lists:filter(
	  fun({_, {smf_aaa_charging, Fn, _}, _})
		when Fn =:= rf_initial; Fn =:= rf_update; Fn =:= rf_terminate ->
...
split_charging1(Config) ->
	...
	fun(BaseTS, Duration, Slot) ->
		#{volume_measurement => #volume_measurement{total = Slot, uplink = 2 * Slot, downlink = 3 * Slot},
		  time_of_first_packet =>
		      #time_of_first_packet{
			 time = smf_gsn_lib:seconds_to_sntp_time(BaseTS + Slot)},
		  time_of_last_packet =>
		      #time_of_last_packet{
			 time = smf_gsn_lib:seconds_to_sntp_time(BaseTS + Duration - Slot)},
		  start_time => #start_time{time = smf_gsn_lib:seconds_to_sntp_time(BaseTS)},
		  end_time => #end_time{time = smf_gsn_lib:seconds_to_sntp_time(BaseTS + Duration)},
		  tp_packet_measurement => #tp_packet_measurement{total = Slot, uplink = 2 * Slot, downlink = 3 * Slot}}
	end,
    ...
    smf_test_sx_up:usage_report('pgw-u01', PCtx, OnMatchSpec,
    ...
    %% periodic reporting
    smf_test_sx_up:usage_report('pgw-u01', PCtx, OffMatchSpec,
    ...
    ChargingH = meck:history(smf_aaa_charging),
    RfInv =
	lists:filter(
	  fun({_, {smf_aaa_charging, Fn, _}, _})
		when Fn =:= rf_initial; Fn =:= rf_update; Fn =:= rf_terminate ->
...
pfcp_select(Config) ->
    APN_FQDN = smf_node_selection:apn_to_fqdn([<<"multi-vrf">>]),
    ...
    Candidates = smf_node_selection:topology_select(APN_FQDN, [], Services, NodeSelect),
    SxConnectId = smf_sx_node:request_connect(Candidates, NodeSelect, 1000),
    smf_sx_node:wait_connect(SxConnectId),

    {ok, {UPinfo, SOpts0}} = smf_pfcp_context:select_upf(Candidates, #{}, APNOpts),

    SOpts = SOpts0#{'NAT-Pool-Id' => <<"nat-E">>},
    Res = smf_pfcp_context:reselect_upf(Candidates, SOpts, APNOpts, UPinfo),
    ct:pal("Res: ~p~n", [Res]),
...
aa_pool_select(Config) ->
    ...
    ok = meck:expect(smf_aaa_auth, authenticate,
		     fun (Ctx, Session, SOpts, Opts) ->
			     {_, Ctx1, SessIn, EvIn} =
				 meck:passthrough([Ctx, Session, SOpts, Opts]),
			     SOut = maps:merge(SessIn, AAAReply),
			     {ok, Ctx1, SOut, EvIn}
		     end),
...
aa_nat_select(Config) ->
    ...
    [SInv|_] =
	lists:foldr(
	  fun({_, {smf_aaa_auth, start, [_, _, _, _]}, {ok, _Ctx, Session_S, _}}, Acc) ->
		  [Session_S|Acc];
	     (_, Acc) ->
		  Acc
	  end, [], H),
	...
	fun({online, Id}, IEs) ->
		IEbef =
		    #{urr_id => #urr_id{id = Id},
		      ...
		      start_time => #start_time{time = 3775809600},
		      end_time => #end_time{time = 3775810200},
		      time_of_first_packet => #time_of_first_packet{time = 3775809600 + 30},
		      time_of_last_packet => #time_of_last_packet{time = 3775810200 - 30},
		      duration_measurement => #duration_measurement{duration = 600},
		      volume_measurement => #volume_measurement{total = 5, uplink = 2, downlink = 3},
		      tp_packet_measurement => #tp_packet_measurement{total = 12, uplink = 5, downlink = 7}},
		IEaft =
		    #{urr_id => #urr_id{id = Id},
		      ...
		      start_time => #start_time{time = 3775810200},
		      end_time => #end_time{time = 3775810800},
		      time_of_first_packet => #time_of_first_packet{time = 3775810200 + 30},
		      time_of_last_packet => #time_of_last_packet{time = 3775810800 - 30},
		      duration_measurement => #duration_measurement{duration = 600},
		      volume_measurement => #volume_measurement{total = 20, uplink = 9, downlink = 11},
		      tp_packet_measurement => #tp_packet_measurement{total = 28, uplink = 13, downlink = 15}},
		[IEbef, IEaft | IEs];
	   ({offline, Id}, IEs) ->
		IEbef =
		    #{urr_id => #urr_id{id = Id},
...
gy_ccr_asr_overlap(Config) ->
    ...
    CCR =
	lists:filter(
	  fun({_, {smf_aaa_charging, F, _}, _})
		when F == gy_ccr_initial; F == gy_ccr_update; F == gy_ccr_terminate ->
		  true;
	     (_) ->
		  false
	  end, H),
...
volume_threshold(Config) ->
    ...
    smf_test_sx_up:usage_report('pgw-u01', PCtx, MatchSpec, #{usage_report_trigger => #{'VOLTH' => []}}),
    smf_test_sx_up:usage_report('pgw-u01', PCtx, MatchSpec, #{usage_report_trigger => #{'VOLQU' => []}}),
...
gx_rar_gy_interaction(Config) ->
    ...
    ?equal(ok, Resp2),
    {ok, PCR2} = smf_context:test_cmd(gtp, CtxKey, pcc_rules),
    ?match(#{<<"r-0001">> := #{}}, PCR2),
...
gx_rar(Config) ->
    ...
    ?equal(ok, Resp0),
    {ok, PCR0} = smf_context:test_cmd(gtp, CtxKey, pcc_rules),
    ?match(#{<<"r-0001">> := #{}}, PCR0),
    ...
    ?equal(ok, Resp3),
    {ok, PCR3} = smf_context:test_cmd(gtp, CtxKey, pcc_rules),
    ?match(#{<<"r-0001">> := #{},
    ...
    ?equal(ok, Resp4),
    {ok, PCR4} = smf_context:test_cmd(gtp, CtxKey, pcc_rules),
    ?match(#{<<"r-0001">> := #{}}, PCR4),
    ?equal(false, maps:is_key(<<"r-0002">>, PCR4)),

    delete_session(GtpC),

    H = smf_test_sx_up:history('pgw-u01'),
    ct:pal("H: ~s~n",
    ...
    %% TBD, should be 5
    %% ?match(2, meck:num_calls(smf_aaa_charging, gy_ccr_update,
    %% 			     ['_', '_', '_', '_'])),

    Hs = meck:history(smf_aaa_charging),
    CCR =
	lists:filter(
	  fun({_, {smf_aaa_charging, gy_ccr_update, [_, _, _, _]}, _}) ->
		  true;
	     (_) ->
		  false
	  end, Hs),
...
apn_lookup(_Config) ->
    ...
    ct:pal("VRF: ~p", [smf_gsn_lib:select_vrf(NodeCaps, ?'APN-EXAMPLE', [])]),
    ?match({ok, <<3, "sgi">>}, smf_gsn_lib:select_vrf(NodeCaps, ?'APN-EXAMPLE', [])),
    ?match({ok, <<3, "sgi">>}, smf_gsn_lib:select_vrf(NodeCaps, [<<"exa">>, <<"mple">>, <<"net">>], [])),
    ?match({ok, <<3, "sgi">>}, smf_gsn_lib:select_vrf(NodeCaps, [<<"APN1">>], [])),
    ?match({ok, <<3, "sgi">>}, smf_gsn_lib:select_vrf(NodeCaps, [<<"APN1">>, <<"mnc001">>, <<"mcc001">>, <<"gprs">>], [])),
    ?match({ok, <<3, "sgi">>}, smf_gsn_lib:select_vrf(NodeCaps, [<<"APN2">>], [])),
    ?match({ok, <<3, "sgi">>}, smf_gsn_lib:select_vrf(NodeCaps, [<<"APN2">>, <<"mnc001">>, <<"mcc001">>, <<"gprs">>], [])),
...
gx_invalid_charging_rulebase(Config) ->
    ...
    ?match([#{tunnels := 1}], [X || X = #{version := Version} <- smf_api:peer(ClientIP),
				    Version == v2]),
...
up_inactivity_timer(Config) ->
    ...
    ok = meck:expect(
	   smf_aaa_auth, authenticate,
	   fun (Ctx, Sess, SOpts, Opts) ->
		   {_, CtxIn, SessIn, EvIn} =
		       meck:passthrough([Ctx, Sess, SOpts, Opts]),
		   {SOut, EvOut} =
    ...
    smf_test_sx_up:up_inactivity_timer_expiry('pgw-u01', PCtx),
...
modify_bearer_command_arp_fanout() ->
    [{doc, "TS 23.401 5.4.2.2 step 5: a Modify Bearer Command that changes the "
	   ...
	   "bearer that carried the previously subscribed ARP"}].
modify_bearer_command_arp_fanout(Config) ->
    Cntl = whereis(gtpc_client_server),
    CtxKey = #context_key{socket = 'irx-socket', id = {imsi, ?'IMSI', 5}},
...
%%--------------------------------------------------------------------
gx_rar_dedicated_bearer_modify() ->
    [{doc, "TS 23.401 5.4.2.1 / 5.4.3: a second Gx RAR that binds an additional "
	   "PCC rule onto an existing dedicated bearer must emit a network-initiated "
	   "Update Bearer Request (not a Create/Delete) with the aggregated QoS and "
	   "merged TFT for that bearer's EBI"}].
gx_rar_dedicated_bearer_modify(Config) ->
    Cntl = whereis(gtpc_client_server),
    CtxKey = #context_key{socket = 'irx-socket', id = {imsi, ?'IMSI', 5}},
    ...
    %% bearer. This must NOT create/delete a bearer but modify the existing one:
    %% the PGW aggregates GBR/MBR (Σ) and merges the TFT into an Update Bearer
    %% Request for DedEBI.
    ...
    AAAReq2 = #aaa_request{from = ResponseFun, procedure = {gx, 'RAR'},
			   session = SessionOpts2, events = [{pcc, install, [Rule2]}]},
    Server ! AAAReq2,
    ...
    %% Expect a network-initiated Update Bearer Request for DedEBI with the
    %% aggregated GBR/MBR (6+4=10 UL, 8+5=13 DL) and a TFT.
		    ...
		    #v2_bearer_context{
		       group = #{{v2_eps_bearer_id, 0} :=
				 ...
				 {v2_bearer_level_quality_of_service, 0} :=
				     #v2_bearer_level_quality_of_service{
					pl = 2, label = 1,
					guaranteed_bit_rate_for_uplink = 10,
					guaranteed_bit_rate_for_downlink = 13,
					maximum_bit_rate_for_uplink = 10,
					maximum_bit_rate_for_downlink = 13},
...
%% Build a delete_bearer_response carrying one Bearer Context per {EBI, Cause}
%% (TS 29.274 7.2.10.2: every EBI the request named shall be echoed).
delete_bearer_response(#gtp{seq_no = SeqNo}, #gtpc{restart_counter = RCnt,
						  remote_control_tei = RemoteCntlTEI},
		       MsgCause, BearerCauses) ->
    BearerCtxs = [#v2_bearer_context{
		     group = [#v2_eps_bearer_id{eps_bearer_id = EBI},
			      #v2_cause{v2_cause = Cause}]}
		  || {EBI, Cause} <- BearerCauses],
    IEs = [#v2_recovery{restart_counter = RCnt},
	   #v2_cause{v2_cause = MsgCause} | BearerCtxs],
...
establish_ded_bearer_with_pf(GtpC, Cntl, CtxKey) ->
    ...
    %% Install by Charging-Rule-Definition with a bare (not list-wrapped)
    %% Charging-Rule-Name, so the rule is keyed under <<"ded-rule-1">> -- the
    %% same value a later Charging-Rule-Remove names (mirrors the codebase's
    %% Initial-Gx-Redirect answer). The SDF filter carries a Gx
    %% Packet-Filter-Identifier so the bearer's sdf_to_pf is populated.
    DedRule = #{'Charging-Rule-Definition' =>
                    [#{'Charging-Rule-Name' => <<"ded-rule-1">>,
                       'Flow-Information' =>
                           [#{'Flow-Description' => [<<"permit out ip from any to assigned">>],
...
%%--------------------------------------------------------------------
ue_delete_packet_filters_deletes_bearer() ->
    [{doc, "Check that a Bearer Resource Command whose TAD deletes all packet "
      "filters of a dedicated bearer drives a Gx CCR-Update, and once the PCRF "
      "removes the bearer's last rule, a single Delete Bearer Request echoing "
      "the PTI is emitted (TS 23.401 5.4.5)"}].
ue_delete_packet_filters_deletes_bearer(Config) ->
    Cntl = whereis(gtpc_client_server),
    CtxKey = #context_key{socket = 'irx-socket', id = {imsi, ?'IMSI', 5}},
    ...
    smf_test_lib:load_aaa_answer_config([{{gx, 'CCR-Update'}, 'Update-Gx-Remove-Ded'}]),

    PTI = 8,
    {_GtpC2, _BRCReq} =
        bearer_resource_command({delete_pf, DedEBI, PfIds, PTI}, GtpC),

    %% The PGW parks on the CCR-U await, the mock removes the rule, the bearer's
    %% last rule is gone -> exactly one Delete Bearer Request on the control
    %% socket carrying the dedicated EBI (instance 1) and echoing the UE PTI.
    DBReq = recv_pdu(Cntl, 5000),
    ?match(#gtp{type = delete_bearer_request,
                ie = #{{v2_eps_bearer_id, 1} :=
                           #v2_eps_bearer_id{eps_bearer_id = DedEBI},
                       {v2_procedure_transaction_id, 0} :=
                           #v2_procedure_transaction_id{pti = PTI}}}, DBReq),
...
ue_delete_packet_filters_reject(Config) ->
    ...
    %% The rejection short-circuits to a Bearer Resource Failure Indication
    %% carrying the mandatory Cause, Linked EPS Bearer ID and echoed PTI.
    Resp = recv_pdu(GtpC2, BRCReq#gtp.seq_no, 5000, ok),
    ?match(#gtp{type = bearer_resource_failure_indication,
                ie = #{{v2_cause, 0} :=
                           #v2_cause{v2_cause = request_rejected},
...
%%--------------------------------------------------------------------
ue_replace_packet_filters_updates_bearer() ->
    [{doc, "Check that a Bearer Resource Command whose TAD replaces an existing "
      "packet filter's content drives a Gx CCR-Update MODIFICATION and, when the "
      "PCRF reinstalls the same rule with the changed flow, emits a single Update "
      "Bearer Request echoing the PTI carrying the changed (still one-filter) TFT "
      "rather than a Delete or Create Bearer Request (TS 23.401 5.4.5, #22 Inc5)"}].
ue_replace_packet_filters_updates_bearer(Config) ->
    Cntl = whereis(gtpc_client_server),
    CtxKey = #context_key{socket = 'irx-socket', id = {imsi, ?'IMSI', 5}},
    ...
    %% one filter's content) without gaining or losing a filter.
    smf_test_lib:load_aaa_answer_config([{{gx, 'CCR-Update'}, 'Update-Gx-Replace-One'}]),

    PTI = 12,
    %% Replace the existing filter (named by its current TFT id) with new content.
    FlowSpec = #{id => hd(PfIds), direction => downlink, precedence => 90,
                 components => [{protocol, 17}, {remote_port, 4000}]},
    {_GtpC2, _BRCReq} =
        bearer_resource_command({replace_pf, DedEBI, FlowSpec, PTI}, GtpC),

    %% The PGW parks on the CCR-U MODIFICATION await, the mock reinstalls the rule
    %% with the new flow, the descriptor's tft changes -> re-provision PFCP (UPF
    %% mock auto-answers), then exactly one Update Bearer Request echoing the PTI
    %% with the changed (still one-filter) TFT.
    UBReq = recv_pdu(Cntl, 5000),
    ?match(#gtp{type = update_bearer_request,
                ie = #{{v2_procedure_transaction_id, 0} :=
                           #v2_procedure_transaction_id{pti = PTI},
                    ...
                    #v2_bearer_context{
                       group = #{{v2_eps_bearer_level_traffic_flow_template, 0} :=
                                     #v2_eps_bearer_level_traffic_flow_template{
                                        value = TFTBin}}}}} = UBReq,
    #{filters := TFTFilters} = smf_tft:decode(TFTBin),
    %% a replace keeps the filter count (one in, one out) -> still one filter
    ?equal(1, length(TFTFilters)),
...
mme_delete_bearer_command_partial_reject() ->
    [{doc, "TS 29.274 7.2.17: an MME-Initiated Delete Bearer Command naming "
      ...
      "network-initiated Delete Bearer Request, not two separate messages "
      "(TS 23.401 5.4.4.2)"}].
mme_delete_bearer_command_partial_reject(Config) ->
    Cntl = whereis(gtpc_client_server),
    CtxKey = #context_key{socket = 'irx-socket', id = {imsi, ?'IMSI', 5}},
    ...
    %% Answer with a per-bearer Cause: the peer deleted the first bearer and
    %% refused the second. Only the accepted one may be released -- the refused
    %% bearer still exists at the peer, so dropping it locally would diverge.
    DBResp = delete_bearer_response(DBReq, GtpC2, request_accepted_partially,
				    [{DedEBI1, request_accepted},
				     {DedEBI2, no_resources_available}]),
    send_pdu(Cntl, GtpC2, DBResp),
