...
lib_init_per_group(Config0) ->
    ...
    meck_init(Config),
    load_config(AppCfg),
    {ok, _} = application:ensure_all_started(smf_core),
    smf_cluster:wait_till_ready(),
    ok = smf_cluster:start([{enabled, false}]),
    smf_cluster:wait_till_running(),

    init_apps(AppCfg),

    case proplists:get_value(upf, Config, true) of
	true ->
	    {ok, _} = smf_test_sx_up:start('pgw-u01', proplists:get_value(pgw_u01_sx, Config)),
	    {ok, _} = smf_test_sx_up:start('pgw-u02', proplists:get_value(pgw_u02_sx, Config)),
	    {ok, _} = smf_test_sx_up:start('sgw-u', proplists:get_value(sgw_u_sx, Config));
	_ ->
	    ok = smf_test_sx_up:stop('pgw-u01'),
	    ok = smf_test_sx_up:stop('pgw-u02'),
	    ok = smf_test_sx_up:stop('sgw-u')
    end,
    {ok, AppsCfg} = application:get_env(smf_aaa, apps),
...
lib_end_per_group(Config) ->
    ...
    [application:stop(App) || App <- [ranch, cowboy, smf_core, smf_aaa, smf_cluster]],
    ok.

init_apps(AppCfg) ->
    init_app(smf_aaa, fun smf_aaa_init/1, AppCfg),
    init_app(smf_core, fun smf_core_init/1, AppCfg),
    ok.
...
load_config(AppCfg) when is_list(AppCfg) ->
    lists:foreach(
      fun({App, _}) when App =:= smf_core; App =:= smf_aaa ->
	      clear_app_env(App),
	      ok;
...
update_app_config(Group, CfgUpd, Config0) ->
	...
	lists:foldl(
	  fun({AppKey, CfgKey}, AppCfg) ->
		  set_cfg_value([smf_core] ++ AppKey, update_app_cfgkey(CfgKey, Config), AppCfg)
	  end, AppCfg0, CfgUpd),
    CoreCfg = validate_core_config(proplists:get_value(smf_core, AppCfg1)),
    AppCfg = lists:keystore(smf_core, 1, AppCfg1, {smf_core, CoreCfg}),
    lists:keystore(app_cfg, 1, Config, {app_cfg, AppCfg}).


init_app(App, Fun, Cfg) ->
    Config = proplists:get_value(App, Cfg),
    Fun(smf_core_config:to_map(Config)).

smf_core_init(Config) ->
    Init = [node, wait_till_running, path_management, node_selection,
	    sockets, upf_nodes, handlers, ip_pools, apns, charging],
    lists:foreach(smf_core_init(_, Config), Init).

smf_core_init(node, #{node := Node}) ->
    smf_core:start_node(Node);
smf_core_init(wait_till_running, _) ->
    smf_core:wait_till_running();
smf_core_init(path_management, #{path_management := NodeSel}) ->
    ok = smf_core:setopts(path_management, NodeSel);
smf_core_init(node_selection, #{node_selection := NodeSel}) ->
    ok = smf_core:setopts(node_selection, NodeSel);
smf_core_init(sockets, #{sockets := Sockets}) ->
    maps:map(fun smf_core:add_socket/2, Sockets);
smf_core_init(upf_nodes, #{upf_nodes := UP}) ->
    smf_core:setopts(sx_defaults, maps:get(default, UP, #{})),
    maps:map(fun smf_core:add_sx_node/2, maps:get(nodes, UP, #{}));
smf_core_init(handlers, #{handlers := Handlers}) ->
    maps:map(fun smf_core:add_handler/2, Handlers);
smf_core_init(ip_pools, #{ip_pools := Pools}) ->
    maps:map(fun smf_core:add_ip_pool/2, Pools);
smf_core_init(apns, #{apns := APNs}) ->
    maps:map(fun smf_core:add_apn/2, APNs);
smf_core_init(charging, #{charging := Charging}) ->
    Init = [rules, rulebase, profile],
    lists:foreach(smf_charging_init(_, Charging), Init);
smf_core_init(_, _) ->
    ok.

smf_charging_init(rules, #{rules := Rules}) ->
    maps:map(fun smf_core:add_charging_rule/2, Rules);
smf_charging_init(rulebase, #{rulebase := RuleBase}) ->
    maps:map(fun smf_core:add_charging_rulebase/2, RuleBase);
smf_charging_init(profiles, #{profiles := Profiles}) ->
    maps:map(fun smf_core:add_charging_profile/2, Profiles);
smf_charging_init(_, _) ->
    ok.

%% copied from smf_aaa test suites
smf_aaa_init(Config) ->
    Init = [product_name, rate_limits, handlers, services, functions, apps],
    lists:foreach(smf_aaa_init(_, Config), Init).

smf_aaa_init(product_name, #{product_name := PN0}) ->
    PN = smf_aaa_config:validate_option(product_name, PN0),
    smf_aaa:setopt(product_name, PN),
    PN;
smf_aaa_init(rate_limits, #{rate_limits := Limits0}) ->
    Limits1 = maps:with([default], Limits0),
    Limits = maps:merge(Limits1, maps:get(peers, Limits0, #{})),
    lists:foreach(smf_aaa:setopt(rate_limit, _), maps:to_list(Limits));
smf_aaa_init(handlers, #{handlers := Handlers0}) ->
    Handlers = smf_aaa_config:validate_options(
		 fun smf_aaa_config:validate_handler/2, Handlers0, []),
    maps:map(fun smf_aaa:add_handler/2, Handlers),
    Handlers;
smf_aaa_init(services, #{services := Services0}) ->
    Services = smf_aaa_config:validate_options(
		 fun smf_aaa_config:validate_service/2, Services0, []),
    maps:map(fun smf_aaa:add_service/2, Services),
    Services;
smf_aaa_init(functions, #{functions := Functions0}) ->
    Functions = smf_aaa_config:validate_options(
		  fun smf_aaa_config:validate_function/2, Functions0, []),
    maps:map(fun smf_aaa:add_function/2, Functions),
    Functions;
smf_aaa_init(apps, #{apps := Apps0}) ->
    Apps = smf_aaa_config:validate_options(fun smf_aaa_config:validate_app/2, Apps0, []),
    maps:map(fun smf_aaa:add_application/2, Apps),
    Apps;
smf_aaa_init(_K, _) ->
...
validate_core_config(Config) ->
    smf_core_config:validate_options(fun validate_option/2, Config, []).

validate_option(node, Value) when ?is_opts(Value) ->
    smf_core:validate_options(Value);
validate_option(sockets, Value) when ?is_opts(Value) ->
    smf_core_config:validate_options(fun smf_socket:validate_options/2, Value, []);
validate_option(handlers, Value) when ?non_empty_opts(Value) ->
    smf_core_config:validate_options(fun smf_context:validate_options/2, Value, []);
validate_option(node_selection, Values) when ?is_opts(Values) ->
    smf_node_selection:validate_options(Values);
validate_option(upf_nodes, Values) when ?is_opts(Values) ->
    smf_core_config:validate_options(fun validate_upf_nodes/2, Values, [{default, #{}}, {nodes, #{}}]);
validate_option(ip_pools, Value) when ?is_opts(Value) ->
    smf_core_config:validate_options(fun smf_ip_pool:validate_options/2, Value, []);
validate_option(apns, Value) when ?is_opts(Value) ->
    smf_core_config:validate_options(fun smf_apn:validate_options/1, Value, []);
validate_option(charging, Opts) ->
    smf_core_config:validate_options(fun validate_charging_options/2, Opts, []);
validate_option(path_management, Opts) when ?is_opts(Opts) ->
    gtp_path:validate_options(Opts);
validate_option(metrics, Opts) ->
    smf_prometheus:validate_options(Opts);
validate_option(Opt, Value) ->
    erlang:error(badarg, [Opt, Value]).

validate_upf_nodes(default, Values) ->
    smf_sx_node:validate_defaults(Values);
validate_upf_nodes(nodes, Nodes) ->
    smf_core_config:validate_options(
      fun smf_sx_node:validate_options/2, Nodes, []);
validate_upf_nodes(Opt, Value) ->
    erlang:error(badarg, [Opt, Value]).

validate_charging_options(profiles, Profiles) ->
    smf_core_config:validate_options(fun smf_charging:validate_profile/2, Profiles, []);
validate_charging_options(rules, Rules) ->
    smf_core_config:validate_options(fun smf_charging:validate_rule/2, Rules, []);
validate_charging_options(rulebase, RuleBase) ->
    smf_core_config:validate_options(fun smf_charging:validate_rulebase/2, RuleBase, []).
...
f_teid(TEID, {_,_,_,_} = IP) ->
    #f_teid{teid = TEID, ipv4 = smf_inet:ip2bin(IP)};
f_teid(TEID, {_,_,_,_,_,_,_,_} = IP) ->
    #f_teid{teid = TEID, ipv6 = smf_inet:ip2bin(IP)}.
...
reconnect_all_sx_nodes() ->
    SxNodes = supervisor:which_children(smf_sx_node_sup),
    [smf_sx_node:test_cmd(Pid, reconnect) || {_, Pid, _, _} <- SxNodes, is_pid(Pid)],
    timer:sleep(10),
    sx_nodes_up(length(SxNodes), 20),
    ok.
...
stop_all_sx_nodes() ->
    SxNodes = supervisor:which_children(smf_sx_node_sup),
    [smf_sx_node:test_cmd(Pid, stop) || {_, Pid, _, _} <- SxNodes, is_pid(Pid)],
    stop_all_sx_nodes(supervisor:which_children(smf_sx_node_sup)).
...
sx_nodes_up(_N, 0) ->
    Got = smf_sx_node_reg:available(),
    Expected = supervisor:which_children(smf_sx_node_sup),
    ct:fail("Sx nodes failed to come up~nExpected ~p~nGot ~p",
	    [Expected, Got]);
sx_nodes_up(N, Cnt) ->
    case smf_sx_node_reg:available() of
...
wait4tunnels(Cnt) ->
    case [X || X = #{tunnels := T} <- smf_api:peer(all), T /= 0] of
	[] -> ok;
	Other ->
	    if Cnt > 100 ->
		    ct:sleep(100),
...
set_online_charging(Set) ->
    {ok, Cfg0} = smf_core_config:get([charging_rule], undefined),
    Cfg = set_online_charging(['_'], Set, Cfg0),
    ok = smf_core_config:put(charging_rule, Cfg).

%% Set APN key data
set_apn_key(Key, Value) ->
    {ok, APNs0} = smf_core_config:get([apns], undefined),
    Upd = fun(_APN, Val_map) -> maps:put(Key, Value, Val_map) end,
    APNs = maps:map(Upd, APNs0),
    ok = smf_core_config:put(apns, APNs).

% Set Timers for Path management
set_path_timers(SetTimers) ->
    {ok, Timers} = smf_core_config:get([path_management], undefined),
    NewTimers = maps_recusive_merge(Timers, SetTimers),
    ok = smf_core_config:put(path_management, NewTimers).
