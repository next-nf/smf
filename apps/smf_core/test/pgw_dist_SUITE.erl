...
init_per_suite(Config0) ->
	...
	true ->
	    ...
	    Config = smf_dist_test_lib:init_per_suite(Config0),
...
end_per_suite(Config) ->
    smf_dist_test_lib:end_per_suite(Config).

init_per_group(common, Config) ->
    smf_dist_test_lib:init_per_group(Config);
...
end_per_group(Group, Config)
  when Group == common;
       Group == single_socket ->
    ok = smf_dist_test_lib:end_per_group(Config).
...
init_per_testcase(TestCase, Config0) ->
    ...
    smf_dist_test_lib:foreach_node(Config, ?MODULE, node_init_per_testcase, [TestCase, Config]),
    Config.
...
end_per_testcase(TestCase, Config) ->
    ...
    smf_dist_test_lib:foreach_node(Config, Fun),
    Result.
...
master_setup_per_testcase(Config, ClearSxHist) ->
    ...
    smf_dist_test_lib:reconnect_all_sx_nodes(Config),
...
master_end_per_testcase(Config) ->
    ...
    ?match_dist_metric(prometheus_gauge, smf_local_pool_free, PoolId, all, ?IPv4PoolSize),
...
create_session_request_accept_new(Config) ->
    smf_dist_test_lib:foreach_node
      (Config, fun (_) -> smf_core:system_info(accept_new, false) end),
...
path_restart(Config) ->
    ...
    smf_dist_test_lib:wait4contexts(?TIMEOUT, Config),
    ok.
...
path_restart_recovery(Config) ->
    ...
    AllPaths = smf_dist_test_lib:random_node(Config, gtp_path_db, all, []),
    ct:pal("Paths: ~p", [AllPaths]),
    ?match(1, length(AllPaths)),

    AllCtx =
	smf_dist_test_lib:random_node(Config, smf_db, tab2list, [gtp_context_global]),
...
path_restart_multi(Config) ->
    ...
    send_recv_pdu(smf_dist_test_lib:gtp_context_node(2, GtpC4), Echo),
...
simple_session_request(Config) ->
    ...
    ?match_dist_metric(prometheus_gauge, smf_local_pool_free, PoolId, all, ?IPv4PoolSize),
    ?match_dist_metric(prometheus_gauge, smf_local_pool_used, PoolId, all, 0),

    {GtpC, _, _} = create_session(ipv4, Config),

    ?match_dist_metric(prometheus_gauge, smf_local_pool_free, PoolId, one, ?IPv4PoolSize - 1),
    ?match_dist_metric(prometheus_gauge, smf_local_pool_used, PoolId, one, 1),
