...
-import('smf_test_lib', [lib_init_per_group/1,
			  lib_end_per_group/1,
			  update_app_config/3,
			  load_config/1]).
-import('smf_test_lib', [meck_init/1,
			  meck_init_hut_handle_request/1,
			  meck_reset/1,
			  meck_unload/1,
			  meck_validate/1]).
-import('smf_test_lib', [init_seq_no/2,
...
-import('smf_test_lib', [start_gtpc_server/1, start_gtpc_server/2,
...
-import('smf_test_lib', [set_cfg_value/3, add_cfg_value/3]).
		  ...
		  case (Expr) of
		      ...
		      V -> ct:pal("MISMATCH(~s:~b, ~s)~nExpected: ~p~nActual:   ~s~n",
				   [?FILE, ?LINE, ??Expr, ??Guard,
				    smf_test_lib:pretty_print(V)]),
			    error(badmatch)
		  end
	  end)())).

-define(match_map(Expected, Actual), smf_test_lib:match_map(Expected, Actual, ?FILE, ?LINE)).

-define(match_metric(Type, Name, LabelValues, Expected),
	smf_test_lib:match_metric(Type, Name, LabelValues, Expected, ?FILE, ?LINE, 10)).
