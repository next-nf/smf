...
bench_init_per_group(Config0) ->
    ...
    Config = smf_test_lib:init_ets(Config0),
    [application:load(App) || App <- [cowboy, jobs, smf_core, smf_aaa]],
    load_config(AppCfg),
...
init_per_testcase(Config) ->
    ...
    smf_test_sx_up:history('pgw-u01', false),
...
create_nsess(Base, GtpC0, Ctxs, Delay, N) ->
    {Time, {GtpC1, _, _}} =
	timer:tc(smf_pgw_test_lib, create_deterministic_session, [Base, N, GtpC0]),
...
contexts_at_scale(Config) ->
    ...
    [?match(#{tunnels := Tunnels}, X) || X <- smf_api:peer(all)],
