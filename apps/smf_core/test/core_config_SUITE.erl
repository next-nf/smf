...
node(_Config)  ->
    Node = [{node_id, <<"GGSN">>},
	    accept_new],
    ValF = fun smf_core:validate_options/1,
...
gtp_c_socket(_Config)  ->
    ...
    ValF = fun(Values) -> smf_socket:validate_options(name, Values) end,
    ...
    Handler = [{handler, ggsn_gn},
	       ...
	       {aaa, [{'Username',
		       [{default, ['IMSI',   <<"/">>,
				   ...
				   12345,
				   <<"@">>, 'APN']}]}]}],
    ValF = fun(Values) -> smf_context:validate_options(name, Values) end,
...
ip_pool(_Config)  ->
    ...
    ValF = fun(Values) -> smf_ip_pool:validate_options(name, Values) end,
...
apn(_Config)  ->
    ...
    ValF = fun(Values) -> smf_apn:validate_options({?'APN-EXAMPLE', Values}) end,
    ValF2 = fun(Name, Values) -> smf_apn:validate_options({Name, Values}) end,
...
node_sel(_Config)  ->
    ...
    ValF = fun smf_node_selection:validate_options/1,
    ?ok(ValF(NodeSel)),
...
upf_node_defaults(_Config)  ->
    ...
    ValF = fun(Values) -> smf_sx_node:validate_defaults(Values) end,
...
upf_node(_Config)  ->
    Node = [connect],
    ValF = fun smf_sx_node:validate_options/2,
...
metrics(_Config)  ->
    ...
    ValF = fun(Values) -> smf_prometheus:validate_options(Values) end,
...
charging(_Config)  ->
    ValF = fun(Values) -> smf_charging:validate_profile(default, Values) end,
...
charging_rule(_Config)  ->
    ValF = fun smf_charging:validate_rule/2,
...
charging_rulebase(_Config)  ->
    ValF = fun smf_charging:validate_rulebase/2,
