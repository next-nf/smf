...
init_per_suite(Config) ->
    ok = meck:new(smf_core, [passthrough, no_link]),
    ok = meck:expect(smf_core, get_plmn_id, fun() -> {<<"001">>, <<"01">>} end),
    Config.

end_per_suite(_Config) ->
    ok = meck:unload(smf_core),
    ok.
...
init_per_group(dns, Config) ->
	...
	Server when is_list(Server) ->
	    ...
	    ok = smf_inet_res:load_config(NodeSelection),
...
end_per_group(static, Config) ->
    ok = meck:unload(smf_node_selection),
...
srv_lookup(_Config) ->
    R = smf_node_selection:lookup_naptr(<<"example.apn.epc.mnc001.mcc001.3gppnetwork.org">>,
					 ?SERVICES, default),
...
srv_lookup_no_final_a(_Config) ->
    R = smf_node_selection:lookup_naptr(<<"example.apn.epc.mnc002.mcc001.3gppnetwork.org">>,
					 ?SERVICES, default),
    ?match([], R),
    ok.
...
lookup_services_order(_Config) ->
    Services = [{'x-3gpp-ggsn', 'x-gn'}, {'x-3gpp-ggsn', 'x-gp'},
		{'x-3gpp-pgw', 'x-gn'}, {'x-3gpp-pgw', 'x-gp'}],
    R1 = smf_node_selection:lookup_naptr(<<"test-01.apn.epc.mnc001.mcc001.3gppnetwork.org">>,
					 Services, default),
...
a_lookup(_Config) ->
    R = smf_node_selection:lookup_naptr(<<"example.apn.epc.mnc003.mcc001.3gppnetwork.org">>,
					 ?SERVICES, default),
...
a_lookup_no_final_a(_Config) ->
    R = smf_node_selection:lookup_naptr(<<"example.apn.epc.mnc004.mcc001.3gppnetwork.org">>,
					 ?SERVICES, default),
    ?match([], R),
    ok.
...
default_lookup(_Config) ->
    R = smf_node_selection:candidates(<<"example.apn.EPC">>, [{'x-3gpp-upf','x-sxa'}], [default]),
...
topology_match(_Config) ->
    ?match([{{<<"topon.gngp.pgw.south.epc.mnc005.mcc001.3gppnetwork.org">>, _, _, _, _},
	   ...
	   smf_node_selection:topology_match(?L1 ++ ?L2, ?S1)).
...
colocation_match(_Config) ->
    ?match([{{<<"topon.gngp.saegw.south.epc.mnc005.mcc001.3gppnetwork.org">>, _, _, _, _},
	   ...
	   smf_node_selection:colocation_match(?L1 ++ ?L2 ++ ?L3, ?S1 ++ ?S2)).
...
apn_to_fqdn(_Config) ->
    ...
    lists:foreach(
      fun(X) ->
	      ?equal({fqdn,[<<"example">>,<<"com">>,<<"apn">>,<<"epc">>, <<"mnc001">>,<<"mcc001">>,<<"3gppnetwork">>,<<"org">>]},
		     smf_node_selection:apn_to_fqdn(X)) end, TestsOk),
    ...
    ?equal({fqdn,[<<"example">>, <<"gprs">>, <<"apn">>, <<"epc">>, <<"mnc001">>, <<"mcc001">>, <<"3gppnetwork">>, <<"org">>]},
	   smf_node_selection:apn_to_fqdn([<<"example">>, <<"gprs">>])),
    ?equal({fqdn,[<<"apn">>, <<"epc">>, <<"example">>, <<"com">>, <<"3gppnetwork">>, <<"org">>]},
	   smf_node_selection:apn_to_fqdn([<<"example">>, <<"com">>, <<"gprs">>])),

    %% MCC/MNC swapped
    ?equal({fqdn,[<<"example">>, <<"com">>, <<"apn">>, <<"epc">>, <<"mcc001">>, <<"mnc001">>, <<"3gppnetwork">>, <<"org">>]},
	   smf_node_selection:apn_to_fqdn([<<"example">>, <<"com">>, <<"mcc001">>, <<"mnc001">>, <<"gprs">>])),

    %% end with .3gppnetwork.org
    ?equal({fqdn,[<<"example">>, <<"com">>, <<"3gppnetwork">>, <<"org">>]},
	   smf_node_selection:apn_to_fqdn([<<"example">>, <<"com">>, <<"3gppnetwork">>, <<"org">>])),

    %% .3gppnetwork.org with incomplete content
    ?equal({fqdn,[<<"example">>, <<"com">>, <<"mcc001">>, <<"mnc001">>, <<"3gppnetwork">>, <<"org">>]},
	   smf_node_selection:apn_to_fqdn([<<"example">>, <<"com">>, <<"mcc001">>, <<"mnc001">>, <<"3gppnetwork">>, <<"org">>])),

    ?equal({fqdn,[<<"example">>, <<"com">>, <<"apn">>, <<"epc">>, <<"mnc001">>, <<"mcc001">>, <<"3gppnetwork">>, <<"org">>]},
	   smf_node_selection:apn_to_fqdn([<<"example">>, <<"com">>])),

    %% expected to crash
    ?match({'EXIT', {function_clause, _}},
	   (catch smf_node_selection:apn_to_fqdn(["example",<<"com">>]))),
...
lb_entry_lookup(_Config) ->
    R = smf_node_selection:candidates(<<"lb.apn.epc">>, [{'x-3gpp-upf','x-sxa'}], [default]),
    ...
    {N1, R1} = smf_node_selection:snaptr_candidate(R),
    ?match({_, _, _}, N1),
...
lb_entry_stats(_Config) ->
	...
	fun SFun(0,   S) -> S;
	    SFun(Cnt, S) ->
		{{E, _, _}, _} = smf_node_selection:snaptr_candidate(Entries),
...
no_error_empty_data(Config) ->
    ...
    Res1 = smf_inet_res:resolve(Name, default, in, aaaa),
    Cached = smf_inet_res:match(smf_inet_res:make_rr_key(Name, default, in, aaaa)),
    Res2 = smf_inet_res:resolve(Name, default, in, aaaa),
