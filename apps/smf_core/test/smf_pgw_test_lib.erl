...
create_deterministic_session(Base, N, #gtpc{} = GtpC0) ->
    GtpC1 = smf_test_lib:gtp_context_new_teids(Base, N, GtpC0),
...
paa({_,_,_,_} = IP) ->
    #v2_pdn_address_allocation{
       type = ipv4,
       address = smf_inet:ip2bin(IP)
      };
paa({_,_,_,_,_,_,_,_} = IP) ->
    #v2_pdn_address_allocation{
       type = ipv6,
       address = <<64:8, (smf_inet:ip2bin(IP))/binary>>
      };
paa({{_,_,_,_,_,_,_,_} = IP, PrefixLen}) ->
    #v2_pdn_address_allocation{
       type = ipv6,
       address = <<PrefixLen:8, (smf_inet:ip2bin(IP))/binary>>
      }.
...
make_pdn_addr_cfg(static_ipv4, IEs) ->
    RequestedIP = smf_inet:ip2bin(?IPv4StaticIP),
...
make_pdn_addr_cfg(static_ipv6, IEs) ->
    PrefixLen = 64,
    Prefix = smf_inet:ip2bin(?IPv6StaticIP),
    [#v2_pdn_address_allocation{
	type = ipv6,
...
make_pdn_addr_cfg(static_host_ipv6, IEs) ->
    PrefixLen = 128,
    Prefix = smf_inet:ip2bin(?IPv6StaticHostIP),
    [#v2_pdn_address_allocation{
	type = ipv6,
	address = <<PrefixLen, Prefix/binary>>},
     #v2_pdn_type{pdn_type = ipv6},
...
make_pdn_addr_cfg(ipv4, IEs) ->
    RequestedIP = smf_inet:ip2bin({0,0,0,0}),
    [#v2_pdn_address_allocation{type = ipv4,
				address = RequestedIP},
     #v2_pdn_type{pdn_type = ipv4},
     #v2_protocol_configuration_options{
...
make_pdn_addr_cfg(ipv6, IEs) ->
    PrefixLen = 0,
    Prefix = smf_inet:ip2bin({0,0,0,0,0,0,0,0}),
    [#v2_pdn_address_allocation{
	type = ipv6,
	address = <<PrefixLen, Prefix/binary>>},
     #v2_pdn_type{pdn_type = ipv6},
...
make_request(modify_bearer_request, secondary_rat_usage_data_report,
	     #gtpc{restart_counter = RCnt, seq_no = SeqNo,
		   ...
		   remote_control_tei = RemoteCntlTEI,
		   rat_type = RAT}) ->
    ULI = #v2_user_location_information{tai = smf_test_lib:tai(1, 1, 20263),
					ecgi = smf_test_lib:ecgi(1, 1, 138873180)},
    IEs = [#v2_recovery{restart_counter = RCnt},
	   #v2_ue_time_zone{timezone = 10, dst = 0},
	   ULI,
...
make_request(modify_bearer_request, SubType,
  ...
  when SubType == simple; SubType == ra_update ->
	...
	case SubType of
	    ra_update ->
		#v2_user_location_information{tai = smf_test_lib:tai(1, 1, SeqNo band 16#ffff),
					      ecgi = smf_test_lib:ecgi(1, 1, 138873180)};
	    _ ->
		#v2_user_location_information{tai = smf_test_lib:tai(1, 1, 55001),
					      ecgi = smf_test_lib:ecgi(1, 1, 138873180)}
...
make_request(bearer_resource_command, {delete_pf, TargetEBI, PfIds, PTI},
	     #gtpc{restart_counter = RCnt, seq_no = SeqNo,
		   local_ip = LocalIP,
		   local_control_tei = LocalCntlTEI,
		   remote_control_tei = RemoteCntlTEI}) ->
    %% UE requests removal of specific packet filters (named by TFT packet-filter
    %% id) from an existing dedicated bearer (delete_packet_filters, TS 24.008
    %% §10.5.6.12 / TS 23.401 §5.4.5).
    TADBin = smf_tft:encode(#{operation => delete_packet_filters,
			      filters => [#{id => Id} || Id <- PfIds],
			      parameters => []}),
    IEs = [#v2_recovery{restart_counter = RCnt},
	   %% Linked EPS Bearer ID (instance 0)
	   #v2_eps_bearer_id{instance = 0, eps_bearer_id = 5},
...
make_request(bearer_resource_command, {add_pf, TargetEBI, FlowSpec, PTI},
	     #gtpc{restart_counter = RCnt, seq_no = SeqNo,
		   local_ip = LocalIP,
		   local_control_tei = LocalCntlTEI,
		   remote_control_tei = RemoteCntlTEI}) ->
    %% UE requests addition of a new packet filter (full content, TS 24.008
    %% §10.5.6.12 / TS 23.401 §5.4.5) to an existing dedicated bearer.
    TADBin = smf_tft:encode(#{operation => add_packet_filters,
			      filters => [FlowSpec],
			      parameters => []}),
    IEs = [#v2_recovery{restart_counter = RCnt},
	   %% Linked EPS Bearer ID (instance 0)
...
make_response(#gtp{type = create_session_request, seq_no = SeqNo},
    ...
    IEs = #{{v2_cause,0} => #v2_cause{v2_cause = request_accepted},
		...
		#v2_protocol_configuration_options{
		   config = {0, [{ipcp,'CP-Configure-Nak',0,
				  [{ms_dns1, smf_inet:ip2bin({8,8,8,8})},
				   {ms_dns2, smf_inet:ip2bin({8,8,4,4})}]},
				 {13, smf_inet:ip2bin({8,8,4,4})},
				 {13, smf_inet:ip2bin({8,8,8,8})}]}},
...
validate_response(create_session_request, static_ipv6, Response, GtpC0) ->
    ...
    ?equal(64, PrefixLen),
    ?equal(?IPv6StaticIP, smf_inet:bin2ip(IPv6)),
    GtpC;
