...
setopts(Opts0) ->
    ...
    smf_core_config:put(path_management, Opts).

getopts() ->
    case smf_core_config:get([path_management], []) of
...
set_peer_opts(Peer, Opts0) when ?IS_IP(Peer) ->
    Opts = validate_options(Opts0),
    {ok, Peers} = smf_core_config:get([gtp_peers], #{}),
    smf_core_config:put(gtp_peers, Peers#{Peer => Opts}).
...
validate_options(Values) ->
    smf_core_config:validate_options(fun validate_option/2, Values, ?Defaults).
...
validate_state(_State, events, Values) ->
    smf_core_config:validate_options(fun validate_event_opts/2, Values, ?EventDefaults);
...
validate_option(Opt = idle, Values) ->
    smf_core_config:validate_options(validate_state(Opt, _, _), Values, ?IdleDefaults);
validate_option(Opt = busy, Values) ->
    smf_core_config:validate_options(validate_state(Opt, _, _), Values, ?BusyDefaults);
validate_option(Opt = suspect, Values) ->
    smf_core_config:validate_options(validate_state(Opt, _, _), Values, ?SuspectDefaults);
validate_option(Opt = down, Values) ->
    smf_core_config:validate_options(validate_state(Opt, _, _), Values, ?DownDefaults);
...
handle_event(cast, {handle_request, ReqKey, #gtp{type = echo_request} = Msg0},
	...
	Msg = #gtp{} ->
	    ...
	    smf_gtp_c_socket:send_response(ReqKey, Response, false),
...
update_contexts(#state{contexts = CtxS} = State,
    ...
    smf_prometheus:gtp_path_contexts(Socket, IP, Version, maps:size(CtxS)),
...
send_echo_request(State, #{socket := Socket, handler := Handler,
    ...
    smf_gtp_c_socket:send_request(Socket, any, DstIP, ?GTP1c_PORT, T3, N3, Msg, CbInfo),
