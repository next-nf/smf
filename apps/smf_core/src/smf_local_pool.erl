...
-module(smf_local_pool).

-behavior(gen_server).
-behavior(smf_ip_pool).
...
start_ip_pool(Name, Opts0) ->
    ...
    smf_ip_pool_sup:start_local_pool_sup(),
    smf_local_pool_sup:start_ip_pool(Name, Opts).
...
send_pool_request(ClientId, {Pool, IP, PrefixLen, Opts}) ->
    send_request(smf_local_pool_reg:lookup(Pool), {get, ClientId, IP, PrefixLen, Opts}).
...
validate_options(Options) ->
    ...
    smf_core_config:validate_options(fun validate_option/2, Options, ?DefaultOptions).
...
validate_ip_range(Opts) when is_list(Opts) ->
    validate_ip_range(smf_core_config:to_map(Opts));
...
validate_option(Opt, Value)
  when Opt == 'MS-Primary-DNS-Server';   Opt == 'MS-Secondary-DNS-Server';
    ...
    smf_core_config:validate_ip_cfg_opt(Opt, Value);
validate_option(Opt, Pool)
  when Opt =:= 'Framed-Pool';
       Opt =:= 'Framed-IPv6-Pool' ->
    smf_ip_pool:validate_name(Opt, Pool);
...
init([Name, #{ranges := Ranges} = Opts]) ->
    smf_local_pool_reg:register(Name),
...
id2ip(Id, #{'Framed-Interface-Id' := IfId}, #pool{type = ipv6, shift = Shift}) ->
    ...
    smf_inet:ipv6_interface_id(IP, IfId);
...
init_pool(Name, Type, First, Last, Shift) ->
    ...
    prometheus_gauge:declare([{name, smf_local_pool_free},
			      {labels, [name, type, id]},
			      {help, "Free IP addresses in pool"}]),
    prometheus_gauge:declare([{name, smf_local_pool_used},
			      {labels, [name, type, id]},
...
metrics_sync_gauges(#pool{name = Name, type = Type, id = Id,
			  used = Used, free = Free}) ->
    prometheus_gauge:set(smf_local_pool_free, [Name, Type, Id], Free),
    prometheus_gauge:set(smf_local_pool_used, [Name, Type, Id], Used),
    ok.
