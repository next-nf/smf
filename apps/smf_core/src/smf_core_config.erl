...
get_local_key([Key|Next], Default) ->
    case application:get_env(smf_core, Key) of
	{ok, Config} ->
...
put(Key, Val) ->
    case is_global_key(Key) of
	true ->
	    ok = smf_global:put(Key, Val);
	false ->
	    ok = application:set_env(smf_core, Key, Val)
    end.
%%%===================================================================
%%% Get Functions
%%% Get/Put Functions
