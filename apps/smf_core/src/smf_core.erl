...
get_plmn_id() ->
    {ok, #{mcc := MCC, mnc := MNC}} = smf_core_config:get([plmn_id], undefined),
    {MCC, MNC}.
get_node_id() ->
    {ok, Id} = smf_core_config:get([node_id], <<"smf">>),
    Id.
get_accept_new() ->
    {ok, Value} = smf_core_config:get([accept_new], true),
    Value.
get_teid_config() ->
    {ok, Value} = smf_core_config:get([teid], undefined),
    Value.
...
system_info(accept_new, New) when is_boolean(New) ->
    Old = get_accept_new(),
    ok = smf_core_config:put(accept_new, New),
    Old;
system_info(Key, Value) ->
    error(badarg, [Key, Value]).

setopts(node_selection = What, Opts0) ->
    Opts = smf_node_selection:validate_options(Opts0),
...
add_socket(Name, Opts0) ->
    Opts = smf_socket:validate_options(Name, Opts0),
...
add_handler(Name, Opts0) ->
    Opts = smf_context:validate_options(Name, Opts0),
...
add_ip_pool(Name, Opts0) ->
    Opts = smf_ip_pool:validate_options(Name, Opts0),
...
add_sx_node(Name, Opts0) ->
    Opts = smf_sx_node:validate_options(Name, Opts0),
    gen_statem:call(?SERVER, {add_sx_node, Name, Opts}).

do_add_sx_node(Name, Opts) ->
    smf_sx_node:add_sx_node(Name, Opts).
...
validate_options(Opts) when is_map(Opts) ->
    smf_core_config:mandatory_keys([node_id], Opts),
    smf_core_config:validate_options(fun validate_option/2, Opts, ?DefaultOptions);
validate_options(Opts) when is_list(Opts) ->
    validate_options(smf_core_config:to_map(Opts));
...
validate_option(plmn_id, Opts) when is_list(Opts) ->
    validate_option(plmn_id, smf_core_config:to_map(Opts));
...
validate_option(teid, Value) ->
    smf_tei_mngr:validate_option(Value);
...
handle_event({call, From}, {add_socket, Name, Opts}, running, _) ->
    Reply = smf_socket:add_socket(Name, Opts),
...
handle_event({call, From}, {add_ip_pool, Name, Opts}, running, _) ->
    Reply = smf_ip_pool:start_ip_pool(Name, Opts),
    {keep_state_and_data, [{reply, From, Reply}]};
...
load_config(#{plmn_id := PlmnId,
    ...
    ok = smf_core_config:put(plmn_id, PlmnId),
    ok = smf_core_config:put(accept_new, New),
    ok = smf_core_config:put(node_id, NodeId),
    ok = smf_core_config:put(teid, TEID),
    ok.
