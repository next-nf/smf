...
authenticate(Session, SessionOpts) ->
    ...
    case smf_aaa_session:invoke(Session, SessionOpts, authenticate, [inc_session_id]) of
	{ok, SOpts, SEvs} ->
	    {ok, {SOpts, SEvs}};
	Other ->
...
ccr_initial(Session, API, SessionOpts, ReqOpts) ->
    case smf_aaa_session:invoke(Session, SessionOpts, {API, 'CCR-Initial'}, ReqOpts) of
	{ok, SOpts, SEvs} ->
	    {ok, {SOpts, SEvs}};
	{_Fail, _, _} ->
...
usage_report_request(ChargeEv, Now, UsageReport, PCtx, PCC, Session) ->
    ...
    smf_gsn_lib:process_online_charging_events(ChargeEv, GyReqServices, Session, ReqOpts),
...
usage_report(URRActions, UsageReport, PCtx, Session) ->
	...
	{ChargeEv, OldS} ->
	    {_Online, Offline, _} =
		smf_pfcp_context:usage_report_to_charging_events(UsageReport, ChargeEv, PCtx),
	    smf_gsn_lib:process_offline_charging_events(ChargeEv, Offline, Now, OldS, Session);
	_ ->
	    ok
    end.
...
close_context(Reason, UsageReport, PCtx, Session) ->
    ...
    Now = erlang:monotonic_time(),
    ReqOpts = #{now => Now, async => true},
    case smf_aaa_session:invoke(Session, #{}, {gx, 'CCR-Terminate'}, ReqOpts#{async => false}) of
	{ok, _GxSessionOpts, _} ->
