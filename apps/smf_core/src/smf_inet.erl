...
ipv6_interface_id({Prefix, PrefixLen}, IntId) when PrefixLen =< 126 ->
    IP = ipv6_interface_id(smf_inet:ip2bin(Prefix), PrefixLen,
			   smf_inet:ip2bin(IntId)),
    {smf_inet:bin2ip(IP), PrefixLen};
...
ipv6_prefix({Prefix, Len}) ->
    <<IP:Len/bits, _/bitstring>> = smf_inet:ip2bin(Prefix),
    {smf_inet:bin2ip(<<IP/bitstring, 0:(128 - Len)>>), Len}.
