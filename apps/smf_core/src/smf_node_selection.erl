...
topology_select(Candidates, Services, [_|_] = MatchPeers) ->
    ...
    case smf_node_selection:topology_match(Candidates, TopoM) of
	[_|_] = C -> C;
	_ ->
...
validate_options(Opts) when ?is_opts(Opts) ->
    smf_core_config:validate_options(fun validate_options/2, Opts, []);
validate_options(Opts) ->
    erlang:error(badarg, [Opts]).

validate_options(_Name, #{type := static} = Opts) ->
    smf_core_config:mandatory_keys([entries], Opts),
    smf_core_config:validate_options(fun validate_static_option/2, Opts, []);
validate_options(_Name, #{type := dns} = Opts) ->
    smf_core_config:mandatory_keys([server], Opts),
    smf_core_config:validate_options(fun validate_dns_option/2, Opts, [{port, 53}]);
validate_options(Name, Opts) when is_list(Opts) ->
    validate_options(Name, smf_core_config:to_map(Opts));
...
validate_static_entries(#{type := naptr} = Entry) when is_map(Entry) ->
    smf_core_config:mandatory_keys(
      [name, order, preference, service, protocols, replacement], Entry),
    smf_core_config:validate_options(fun validate_naptr/2, Entry, []);
validate_static_entries(#{type := host} = Entry) when is_map(Entry) ->
    smf_core_config:mandatory_keys([name], Entry),
    smf_core_config:validate_options(fun validate_host/2, Entry, [{ip4, []}, {ip6, []}]);
validate_static_entries(Entry) when is_list(Entry) ->
    validate_static_entries(smf_core_config:to_map(Entry));
...
validate_naptr(protocols = Opt, Value) when ?is_non_empty_opts(Value) ->
    smf_core_config:check_unique_elements(Opt, Value),
...
async_lookup_host(Name, Type, Selection) ->
    Owner = self(),
    spawn(
      fun() -> Owner ! {self(), (catch smf_inet_res:resolve(Name, Selection, in, Type))} end).
...
naptr(Name, Selection) ->
    smf_inet_res:resolve(Name, Selection, in, naptr).
...
follow({Host, Order, s, Services}, Selection, Res) ->
    case smf_inet_res:resolve(Host, Selection, in, srv) of
