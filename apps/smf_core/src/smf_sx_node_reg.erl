...
connect(Node, NodeSelect, IP4, IP6, NotifyUp)
  when is_list(NotifyUp) ->
    case lookup(Node) of
	{ok, Pid} = Result ->
	    smf_sx_node:notify_up(Pid, NotifyUp),
	    Result;
	_ ->
...
init([]) ->
    smf_db:create(?SERVER, #{type => ordered_set, scope => local}),
    {ok, #{}}.

handle_register(Pid, Id, _Value, State) ->
    case smf_db:insert_new(?SERVER, {Id, Pid}) of
	true ->  {ok, [Id], State};
...
handle_pid_remove(_Pid, Keys, State) ->
    lists:foreach(fun(Key) -> smf_db:delete(?SERVER, Key) end, Keys),
...
handle_call({connect, Node, NodeSelect, IP4, IP6, NotifyUp}, _From, State) ->
	...
	{ok, Pid} = Result ->
	    ...
	    {reply, Result, State};
	_ ->
	    {ok, Pid, RegKey} = smf_sx_node_sup:new(Node, NodeSelect, IP4, IP6, NotifyUp),
...
unregister(Key, State) ->
    Pids = [Pid || {_, Pid} <- smf_db:take(?SERVER, Key)],
