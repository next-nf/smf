...
-module(smf_prometheus_collector).
...
gather_sx_association() ->
    #{
        [{state, up}] => smf_sx_node_reg:count_available()
    }.

gather_sx_context() ->
    maps:fold(fun(Name, {Pid, _}, Stats) ->
        Stats#{[{name, Name}] => smf_sx_node_reg:count_monitors(Pid)}
    end, #{}, smf_sx_node_reg:available()).
