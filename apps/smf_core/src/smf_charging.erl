...
validate_profile(_Name, Opts)
  when ?is_opts(Opts) ->
    smf_core_config:validate_options(fun validate_charging_options/2, Opts, ?DefaultProfile);
...
validate_rule(Key, Opts)
  when is_binary(Key), ?is_non_empty_opts(Opts) ->
    ...
    smf_core_config:validate_options(fun validate_rule_def/2, Opts, []);
validate_rule(Key, Opts) ->
...
validate_offline_charging_options(triggers, Opts) ->
    smf_core_config:validate_options(fun validate_offline_charging_triggers/2,
...
validate_charging_options(online, Opts) ->
    smf_core_config:validate_options(fun validate_online_charging_options/2,
				 Opts, ?DefaultOnlineChargingOpts);
validate_charging_options(offline, Opts) ->
    smf_core_config:validate_options(fun validate_offline_charging_options/2,
...
add_profile(Name, Opts0) ->
    ...
    {ok, Profiles} = smf_core_config:get([charging_profile], #{}),
    smf_core_config:put(charging_profile, maps:put(Name, Opts, Profiles)).

add_rule(Name, Opts0) ->
    Opts = validate_rule(Name, Opts0),
    {ok, Rules} = smf_core_config:get([charging_rule], #{}),
    smf_core_config:put(charging_rule, maps:put(Name, Opts, Rules)).

add_rulebase(Name, Opts0) ->
    Opts = validate_rulebase(Name, Opts0),
    {ok, RBs} = smf_core_config:get([charging_rulebase], #{}),
    smf_core_config:put(charging_rulebase, maps:put(Name, Opts, RBs)).

%% TODO: use APN, VPLMN, HPLMN and Charging Characteristics
%%       to select config
get_profile() ->
    case smf_core_config:get([charging_profile, default], []) of
	{ok, Opts0} when is_map(Opts0) ->
	    Opts0;
	undefined ->
	    Opts = validate_profile(default, []),
	    smf_core_config:put(charging_profile, #{default => Opts}),
	    Opts
    end.
...
rulebase() ->
    {ok, Rules} = smf_core_config:get([charging_rule], #{}),
    {ok, RuleBase} = smf_core_config:get([charging_rulebase], #{}),
