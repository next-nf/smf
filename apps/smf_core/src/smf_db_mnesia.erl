...
%% @doc Mnesia backend for smf_db.
%%
%% Local tables (scope => local) delegate to smf_db_ets.
...
delete_object(Tab, Obj) ->
	...
	true ->
	    ...
	    ok;
	false ->
	    smf_db_ets:delete_object(Tab, Obj)
    end.
...
update_element(Tab, Key, {Pos, Val}) ->
	...
	false ->
	    smf_db_ets:update_element(Tab, Key, {Pos, Val})
    end;
update_element(Tab, Key, Updates) when is_list(Updates) ->
	...
	false ->
	    smf_db_ets:update_element(Tab, Key, Updates)
    end.
...
select(Tab, MatchSpec) ->
	...
	false ->
	    smf_db_ets:select(Tab, MatchSpec)
    end.
...
select(Tab, MatchSpec, Limit) ->
	...
	false ->
	    smf_db_ets:select(Tab, MatchSpec, Limit)
    end.
