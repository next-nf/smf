...
%% membership, and state. Uses smf_db for raw storage.
...
cas_restart_counter(Key, Counter, Time) ->
    Obj = case smf_db:lookup(?TAB, Key) of
	      [{_, V}] -> V;
	      [] -> init_obj(undefined, #{}, Time)
	  end,
    {Result, New, ObjTime} = do_cas(Counter, Time, Obj),
    smf_db:insert(?TAB, {Key, Obj#{restart_counter => New, time => ObjTime}}),
    {Result, New, ObjTime}.
