...
validate_options(Opts) ->
    smf_core_config:validate_options(fun validate_option/2, Opts, []).
...
%% validate_options(Opts) ->
%%     smf_core_config:validate_options(fun validate_option/2, Opts, ?DefaultMetricsOpts).
...
declare() ->
    %% %% Metrics Config
    %% {ok, Config} = application:get_env(smf_core, metrics),
