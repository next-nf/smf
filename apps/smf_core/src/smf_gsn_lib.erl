...
sntp_time_to_gregorian_seconds(SNTP) ->
    smf_gsn_lib:sntp_time_to_seconds(SNTP) + ?SECONDS_FROM_0_TO_1970.
...
process_online_charging_events(Reason, Request, Session, ReqOpts)
  when is_map(Request) ->
	...
	{terminate, Cause} ->
	    ...
	    smf_aaa_session:invoke(Session, TermReq, {gy, 'CCR-Terminate'}, ReqOpts);
	_ when map_size(Used) /= 0;
	       map_size(Needed) /= 0 ->
	    smf_aaa_session:invoke(Session, Request, {gy, 'CCR-Update'}, ReqOpts);
	_ ->
	    SOpts = smf_aaa_session:get(Session),
	    {ok, SOpts, []}
    end.
...
process_offline_charging_events(ChargeEv, Ev, Now, Session)
  when is_list(Ev) ->
    process_offline_charging_events(ChargeEv, Ev, Now, smf_aaa_session:get(Session), Session).
...
process_offline_charging_events(ChargeEv0, Ev, Now, SessionOpts, Session)
  when is_list(Ev) ->
    ...
    case Reason of
	terminate ->
	    smf_aaa_session:invoke(Session, Request, {rf, 'Terminate'}, SOpts);
	_ when length(SDCs) /= 0; length(TDVs) /= 0 ->
	    smf_aaa_session:invoke(Session, Request, {rf, 'Update'}, SOpts);
	_ ->
	    ok
    end.
...
process_accounting_monitor_events(Reason, Ev, Now, Session)
  when is_list(Ev) ->
    Keys = ['InPackets', 'OutPackets',
	    'InOctets',  'OutOctets',
	    'Session-Start'],
    Update0 = maps:with(Keys, smf_aaa_session:get(Session)),
	...
	{terminate, Cause} ->
	    ...
	    smf_aaa_session:invoke(Session, Update, stop, SOpts);
	_ when Update0 /= Update1 ->
	    Update = maps:remove('Session-Start', Update1),
	    smf_aaa_session:invoke(Session, Update, interim, SOpts);
	_ ->
	    ok
    end.
...
select_vrf(NodeCaps, APN, #{'NAT-Pool-Id' := NAT}) ->
    {ok, #{vrfs := ApnVRFs}} = smf_apn:get(APN),
...
request_ip_alloc(ReqIPs, Opts,# tunnel{local = #fq_teid{teid = TEI}}) ->
    ...
    smf_ip_pool:send_request(TEI, Req).
...
ip_alloc_result(AI, {Result, {UeIP, Opts0}}) ->
    case smf_ip_pool:ip(AI) of
	{IP, _} when ?IS_IPv4(IP) ->
	    Opts1 = maps:merge(Opts0, smf_ip_pool:opts(AI)),
	...
	{IP, _} = IPv6 when ?IS_IPv6(IP) ->
	    Opts1 = maps:merge(Opts0, smf_ip_pool:opts(AI)),
	    Opts = Opts1#{'Framed-IPv6-Prefix' => smf_inet:ipv6_prefix(IPv6)},
...
wait_ip_alloc_results(ReqIds, Opts0) ->
    ...
    lists:foldl(
      fun ip_alloc_result/2, {ok, {#ue_ip{}, Opts}}, smf_ip_pool:wait_response(ReqIds)).
...
allocate_ips_result(ReqPDNType, BearerType, #ue_ip{v4 = MSv4, v6 = MSv6}) ->
    allocate_ips_result(ReqPDNType, BearerType, smf_ip_pool:ip(MSv4),
			smf_ip_pool:ip(MSv6)).
...
icmpv6(TC, FlowLabel, _SrcAddr, ?'IPv6 All Routers LL',
    ...
    PrefixInformation = <<?'RA Prefix Information':8, 4:8,
			  ...
			  (smf_inet:ip2bin(Prefix))/binary>>,

    DNSCnt = length(DNSv6),
    DNSSrvOpt =
	if (DNSCnt /= 0) ->
		<<?'RDNSS', (1 + DNSCnt * 2):8, 0:16, 16#ffffffff:32,
		  << <<(smf_inet:ip2bin(DNS))/binary>> || DNS <- DNSv6 >>/binary >>;
	   true ->
		<<>>
	end,
    ...
    CSum = smf_inet:ip_csum(<<NwSrc:16/bytes-unit:8, NwDst:16/bytes-unit:8,
