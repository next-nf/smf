...
start_ip_pool(Name, #{handler := Handler} = Opts) ->
    {ok, Pools} = smf_core_config:get([ip_pools], #{}),
    smf_core_config:put(ip_pools, maps:put(Name, Opts, Pools)),
...
validate_options(_Name, Values0)
  when ?is_opts(Values0) ->
    Values = smf_core_config:to_map(Values0),
    validate_pool(maps:get(handler, Values, smf_local_pool), Values);
validate_options(Name, Values) ->
    erlang:error(badarg, [Name, Values]).

validate_pool(smf_local_pool, Options) ->
    smf_local_pool:validate_options(Options);
...
with_pool(Pool, Fun) ->
    case smf_core_config:get([ip_pools, Pool], undefined) of
	{ok, #{handler := Handler}} ->
	    Fun(Handler);
	_ ->
	    {error, not_found}
