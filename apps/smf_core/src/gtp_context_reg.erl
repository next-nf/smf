...
lookup(Key) when is_tuple(Key) ->
    case smf_db:lookup(?SERVER, Key) of
	[{Key, Value}] ->
	    Value;
	_ ->
	    undefined
...
select(Key) ->
    smf_db:select(?SERVER, [{{Key, '$1'},[],['$1']}]).

register(Keys, Handler, Pid)
  when is_list(Keys), is_atom(Handler), is_pid(Pid) ->
    register(fun smf_db:insert/2, Keys, Handler, Pid).

register_new(Keys, Handler, Pid)
  when is_list(Keys), is_atom(Handler), is_pid(Pid) ->
    register(fun smf_db:insert_new/2, Keys, Handler, Pid).
...
init([]) ->
    ...
    smf_db:create(?SERVER, #{type => bag, scope => local,
			      decentralized_counters => true,
			      write_concurrency => true, read_concurrency => true}),
    smf_db:create(?GLOBAL_TAB, #{type => set, scope => global}),
    {ok, #{}}.
...
db_delete_objects(Tab, Objects) ->
    [smf_db:delete_object(Tab, Obj) || Obj <- Objects].

add_keys(InsFun, Keys, Handler, Pid) ->
    Insert = mk_reg_keys(Keys, Handler, Pid),
    smf_db:insert(?SERVER, mk_reg_pids(Insert)),
...
global_add_key(#context_key{id = {Type, _, _}} = Key, RegV)
  when Type == imei; Type == imsi ->
    smf_db:insert(?GLOBAL_TAB, {Key, RegV});
global_add_key(_, _) ->
    ok.

global_del_key(#context_key{id = {Type, _, _}} = Key, _RegV)
  when Type == imei; Type == imsi ->
    smf_db:delete(?GLOBAL_TAB, Key);
global_del_key(_, _) ->
    ok.

global_lookup(Key) ->
    case smf_db:lookup(?GLOBAL_TAB, Key) of
	[{_, Value}] -> [Value];
	[] -> []
    end.
