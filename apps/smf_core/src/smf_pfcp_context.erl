...
update_bearer(#f_teid{teid = TEID, ipv4 = IP}, #bearer{local = #fq_teid{ip = v4}} = Bearer) ->
    Bearer#bearer{local = #fq_teid{ip =  smf_inet:bin2ip(IP), teid = TEID}};
...
build_sx_rules(PCC, Opts, PCtx0, BearerMap) ->
    ...
    SxRuleReq = smf_pfcp:update_pfcp_rules(PCtx0, NewPCtx, Opts),
...
build_sx_ctx_rule(AccessBearer, SGiBearer, CpBearer, PCtx0, Update)
  when not is_record(AccessBearer#bearer.remote, ue_ip),
    ...
    FAR = #{far_id => #far_id{id = FarId},
	    apply_action => #{'FORW' => []},
	    forwarding_parameters => smf_pfcp:traffic_forward(CpBearer, #{})
	   },
    Update#sx_upd{
...
build_sx_linked_rule(URR, PCtx) ->
    build_sx_linked_offline_rule(smf_charging:is_enabled(offline), URR, PCtx).

build_sx_linked_offline_rule(true, URR0, PCtx0) ->
    RuleName = {offline, 'IP-CAN'},
    {LinkedUrrId, PCtx} =
	smf_pfcp:get_urr_id(RuleName, ['IP-CAN'], RuleName, PCtx0),
...
build_sx_offline_charging_rule(Name,
    ...
    {UrrId, PCtx1} = smf_pfcp:get_urr_id(ChargingKey, [RatingGroup], ChargingKey, PCtx0),
    ...
    Update#sx_upd{
      pctx = smf_pfcp_rules:add(urr, ChargingKey, URR, PCtx)};
...
build_sx_online_charging_rule(Name, #{'Online' := [1]} = Definition,
    ...
    Update#sx_upd{
      pctx = smf_pfcp_rules:add(urr, ChargingKey, needed, PCtx)};
...
%%	group = smf_pfcp:traffic_forward(Src, [RedirInfo])
%%        }
%%     ];
far(FarId, [#{'Redirect-Support' :=        [1],   %% ENABLED
	      'Redirect-Address-Type' :=   [2],   %% URL
    ...
    #{far_id => #far_id{id = FarId},
      apply_action => #{'FORW' => []},
      forwarding_parameters =>
	  smf_pfcp:traffic_forward(Dst, #{redirect_information => RedirInfo})};
...
far(FarId, _RedirInfo, _Src, Dst) ->
    #{far_id => #far_id{id = FarId},
      apply_action => #{'FORW' => []},
      forwarding_parameters => smf_pfcp:traffic_forward(Dst, #{})}.
...
build_sx_rule(Direction = downlink, Name, Definition, FilterInfo, URRs,
	      AccessBearer, SGiBearer, #sx_upd{pctx = PCtx0} = Update)
  when AccessBearer#bearer.remote /= undefined ->
    ...
    {SGiBearerReq, PCtx} = make_request_bearer(PdrId, SGiBearer, PCtx2),

    PDR = pdr(PdrId, Precedence, dst, SGiBearerReq, AccessBearer, FilterInfo, FarId, URRs),
    FAR = far(FarId, undefined, SGiBearer, AccessBearer),
...
build_sx_rule(Direction = uplink, Name, Definition, FilterInfo, URRs,
    ...
    PDR = pdr(PdrId, Precedence, src, AccessBearerReq, SGiBearer, FilterInfo, FarId, URRs),

    RedirInfo = maps:get('Redirect-Information', Definition, undefined),
    FAR = far(FarId, RedirInfo, AccessBearer, SGiBearer),
...
build_sx_rule(_Direction, Name, _Definition, _FilterInfo, _URRs,
	      _AccessBearer, _SGiBearer, Update) ->
    sx_rule_error({system_error, Name}, Update).
	...
	lists:foldl(
	  fun({K, RG}, Acc) ->
		  case maps:get(K, D, undefined) of
		      [1] -> smf_pfcp:get_urr_group(get_rating_group(RG, D), PCtx) ++ Acc;
		      _   -> Acc
		  end
	  end, smf_pfcp:get_urr_group('IP-CAN', PCtx),
...
build_sx_usage_rule_4(monitoring_time, #{'Tariff-Time-Change' := [TTC]}, _, URR) ->
    Time = smf_gsn_lib:datetime_to_sntp_time(TTC),
...
pfcp_pctx_update(#pfcp_ctx{up_inactivity_timer = UPiTnew} = PCtx,
  ...
  when UPiTold /= UPiTnew ->
    update_m_rec(smf_pfcp:up_inactivity_timer(PCtx), IEs);
...
handle_validity_time(ChargingKey, #{'Validity-Time' := {BaseTime, Time}}, URR, PCtx) ->
    ...
    {URR, smf_pfcp:set_timer(AbsTime, {ChargingKey, validity_time}, PCtx)};
...
%%%        see 3GPP TS 32.251, Sect. 5.2.3.4.1 and Table  5.2.3.4.1.1.
    ...
    %% {URR1, PCtx} =
    %%         case smf_pfcp:get_urr_ids([{offline, RatingGroup}], PCtx2) of
    %%             [undefined] ->
...
build_ipcan_rule(Update) ->
    build_ipcan_rule(smf_charging:is_enabled(offline), Update).
...
build_ipcan_rule(true, #sx_upd{pctx = PCtx0} = Update) ->
    ...
    {UrrId, PCtx} = smf_pfcp:get_urr_id(RuleName, ['IP-CAN'], RuleName, PCtx0),
    ...
    ?LOG(debug, "URR: ~p", [URR]),
    Update#sx_upd{pctx = smf_pfcp_rules:add(urr, RuleName, URR, PCtx)};
...
build_sx_monitor_rule('IP-CAN', Service, {periodic, Time, _Opts} = _Definition,
    ...
    Update#sx_upd{
      pctx = smf_pfcp_rules:add(urr, RuleName, URR, PCtx),
      monitors = Monitors};
...
build_sx_monitor_rule('Offline', Service, {periodic, Time, _Opts} = Definition,
    ...
    %% TBD: Offline Charging Rules at the IP-CAL level need to be seperated
    %%      by Charging-Id (maps to ARP/QCI combi)
    {UrrId, PCtx1} = smf_pfcp:get_urr_id(RuleName, ['IP-CAN'], RuleName, PCtx0),
    ...
    PCtx = smf_pfcp_rules:update_with(urr, RuleName, URRUpd, URR, PCtx1),
...
make_pctx_bearer_key(_, #bearer{local = FqTEID}, PCtx, Keys)
  when is_record(FqTEID, fq_teid), is_integer(FqTEID#fq_teid.teid) ->
    [smf_pfcp:ctx_teid_key(PCtx, FqTEID)|Keys];
...
usage_report_to_charging_events(URR, ChargeEv, PCtx)
  when is_record(PCtx, pfcp_ctx) ->
    UrrIds = smf_pfcp:get_urr_ids(PCtx),
    foldl_usage_report(
...
select_upf_with(Fun, Candidates, Available) ->
    case smf_node_selection:snaptr_candidate(Candidates) of
...
send_g_pdu(PCtx, #bearer{vrf = VRF,
    ...
    UDP = smf_inet:make_udp(
	    smf_inet:ip2bin(SrcIP), smf_inet:ip2bin(DstIP),
	    ?GTP1u_PORT, ?GTP1u_PORT, PayLoad),
    smf_sx_node:send(PCtx, 'Access', VRF, UDP),
    ok.
