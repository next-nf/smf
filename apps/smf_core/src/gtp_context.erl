...
send_request(#tunnel{socket = Socket}, Src, DstIP, DstPort, T3, N3, Msg, ReqInfo) ->
    ...
    smf_gtp_c_socket:send_request(Socket, Src, DstIP, DstPort, T3, N3, Msg, CbInfo).
...
collect_charging_events(OldS0, NewS0) ->
    ...
    case smf_charging:is_charging_event(offline, Events) of
	false ->
	    [];
	ChargeEv ->
...
validate_options(Fun, Opts, Defaults) ->
    smf_core_config:mandatory_keys([protocol, handler, sockets, node_selection], Opts),
    smf_core_config:validate_options(Fun, Opts, Defaults ++ ?ContextDefaults).
...
validate_option(aaa, Value0) when ?is_opts(Value0) ->
    Value = smf_core_config:to_map(Value0),
...
port_message(Request, #gtp{version = v2, type = MsgType, tei = 0} = Msg)
  when MsgType == change_notification_request;
    ...
    smf_context:port_message(Id, Request, Msg);
...
port_message(#request{socket = Socket, info = Info} = Request,
	...
	{ok, Interface, InterfaceOpts} ->
	    case smf_core:get_accept_new() of
		true ->
		    validate_teid(Msg),
...
init({[Socket, Info, Version, Interface,
    ...
    Context = #context{
		 charging_identifier = smf_gtp_c_socket:get_uniq_id(Socket),
...
handle_event({call, From}, {?TestCmdTag, session}, _State, #{aaa_session := Session}) ->
...
handle_event({call, From},
    ...
    Data = smf_gtp_gsn_lib:close_context('pfcp', TermCause, UsageReport, Data0),
...
handle_event({call, From},
	     {sx, #pfcp{type = session_report_request,
	      ...
	      _State, #{pfcp := PCtx, aaa_session := S0,
			charging := C0, aaa_auth := A0, pcc := PCC} = Data) ->
    Now = erlang:monotonic_time(),
    ChargeEv = interim,

    {S1, C1, A1, GyEvs} =
	smf_gtp_gsn_session:usage_report_request(ChargeEv, Now, UsageReport, PCtx, PCC,
						  S0, C0, A0),
    Actions = [{reply, From, {ok, PCtx}} |
	       [{next_event, internal, {session, Ev, S1}} || Ev <- GyEvs]],
    {keep_state, Data#{aaa_session := S1, charging := C1, aaa_auth := A1}, Actions};
...
handle_event(info, #aaa_request{procedure = {_, 'ASR'} = Procedure} = Request,
	     State, #{aaa_session := Session} = Data) ->
    smf_aaa_session:aaa_reply(Request, ok, #{}, Session),
...
handle_event(info, #aaa_request{procedure = {gx, 'RAR'},
    ...
    Events = case Handler of
		 undefined -> ReqEvents;
		 _ ->
		     {_, ToSessEvents} = Handler:to_session({gx, 'RAR'}, {S0, []}, Avps),
...
%%% step 1a:
    {PCC1, _} =
	smf_pcc_context:gx_events_to_pcc_ctx(Events, remove, RuleBase, PCC0),
%%% step 1b:
    {PCC2, PCCErrors2} =
...
handle_event(info, #aaa_request{procedure = {gy, 'RAR'},
				handler = Handler, session = Avps,
				events = ReqEvents} = Request,
	     #{session := connected} = _State, #{aaa_session := S0} = Data) ->
    Events = case Handler of
		 undefined -> ReqEvents;
		 _ ->
		     {_, ToSessEvents} = Handler:to_session({gy, 'RAR'}, {S0, []}, Avps),
		     ReqEvents ++ ToSessEvents
	     end,
    smf_aaa_session:aaa_reply(Request, ok, #{}, S0),
    Now = erlang:monotonic_time(),
...
handle_event(info, {'$reply', Promise, Handler, Msg, _Opts}, _State, Data) ->
	...
	[] ->
	    {keep_state, Data1};
	_ ->
	    Session = maps:get(aaa_session, Data1),
...
send_response(Request, #gtp{seq_no = SeqNo} = Msg) ->
	...
	smf_gtp_c_socket:send_response(Request, Msg, SeqNo /= 0)
    catch
	Class:Error:Stack ->
...
handler_group(smf_aaa_gx) -> pcf;
handler_group(smf_aaa_ro) -> gy;
handler_group(smf_aaa_rf) -> rf;
handler_group(smf_aaa_nasreq) -> auth;
handler_group(smf_aaa_radius) -> auth;
handler_group(_) -> auth.

handle_aaa_reply(Handler, Promise, Msg,
		 #{aaa_session := S0, pcf := PCF0,
		   charging := C0, aaa_auth := A0} = Data) ->
    case handler_group(Handler) of
	pcf ->
	    {PCF1, S1, Events} = smf_aaa_pcf:handle_reply(Promise, Msg, S0, PCF0),
	    {Data#{pcf := PCF1, aaa_session := S1}, Events};
	gy ->
	    {C1, S1, Events} = smf_aaa_charging:handle_reply(Promise, Msg, S0, C0),
	    {Data#{charging := C1, aaa_session := S1}, Events};
	rf ->
	    {C1, S1, Events} = smf_aaa_charging:handle_reply(Promise, Msg, S0, C0),
	    {Data#{charging := C1, aaa_session := S1}, Events};
	auth ->
	    {A1, S1, Events} = smf_aaa_auth:handle_reply(Promise, Msg, S0, A0),
	    {Data#{aaa_auth := A1, aaa_session := S1}, Events}
    end.
...
bsf_keys(APN, _, #bearer{vrf = VRF, local = #ue_ip{v4 = IPv4, v6 = IPv6}}, Keys) ->
    [#bsf{dnn = APN, ip_domain = VRF, ip = smf_ip_pool:ip(IPv4)} || IPv4 /= undefined] ++
	[#bsf{dnn = APN, ip_domain = VRF,
	      ip = smf_inet:ipv6_prefix(smf_ip_pool:ip(IPv6))} || IPv6 /= undefined] ++
    Keys;
bsf_keys(_, _, _, Keys) ->
    Keys.
...
usage_report_fun(Owner, URRActions, PCtx) ->
    case smf_pfcp_context:query_usage_report(offline, PCtx) of
	{ok, {_, Report, _}} ->
