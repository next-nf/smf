...
start_link('gtp-c', Name, Opts) ->
    smf_gtp_c_socket:start_link(Name, Opts);
start_link('gtp-u', Name, Opts) ->
    smf_gtp_u_socket:start_link(Name, Opts);
start_link('pfcp', Name, Opts) ->
    smf_sx_socket:start_link(Name, Opts).

%%
%% Initialize a new PFCP, GTPv1/v2-c or GTPv1-u socket
%%
add_socket(Name, #{type := Type} = Opts) ->
    smf_socket_sup:new(Type, Name, Opts).
...
validate_options(_Name, #{type := Type} = Values)
  when Type =:= 'gtp-c';
       Type =:= 'gtp-u' ->
    smf_gtp_socket:validate_options(Values);
validate_options(_Name, #{type := pfcp} = Values) ->
    smf_sx_socket:validate_options(Values);
validate_options(Name, Values) when is_list(Values) ->
    validate_options(Name, smf_core_config:to_map(Values));
