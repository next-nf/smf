...
validate_options(Values) ->
    smf_core_config:mandatory_keys([ip, socket], Values),
    smf_core_config:validate_options(fun validate_option/2, Values, ?SocketDefaults).
...
init({Name, #{ip := IP, socket := GtpSocketName, burst_size := BurstSize} = Opts}) ->
    ...
    GtpSocket = smf_socket_reg:waitfor('gtp-u', GtpSocketName),
    GtpSockInfo = smf_gtp_socket:info(GtpSocket),

    State = #state{
	       send_socket = SendSocket,
	       recv_socket = RecvSocket,
	       name = Name,
	       node = smf_core:get_node_id(),
...
handle_dest_unreach(IP, [Data|_], #state{name = Name} = State0) when is_binary(Data) ->
    smf_prometheus:pfcp(tx, Name, IP, unreachable),
...
handle_response(ArrivalTS, IP, _Port, #pfcp{seq_no = SeqNo} = Msg,
	...
	none -> %% duplicate, drop silently
	    smf_prometheus:pfcp(rx, Name, IP, Msg, duplicate),
...
handle_request(#sx_request{ip = IP, port = Port} = ReqKey, Msg,
	...
	_Other ->
	    smf_sx_node:handle_request(ReqKey, IP, Msg)
    end,
    State.
...
enqueue_response(ReqKey, Data, DoCache,
  ...
  when DoCache =:= true ->
    State#state{responses =
		    smf_cache:enter(cache_key(ReqKey), Data, ?RESPONSE_TIMEOUT, Responses)};
...
message_counter(Direction, #state{name = Name}, #send_req{address = IP, msg = Msg}) ->
    smf_prometheus:pfcp(Direction, Name, IP, Msg).
...
message_counter(Direction, #state{name = Name}, #send_req{address = IP}, Verdict)
  when is_atom(Verdict) ->
    smf_prometheus:pfcp(Direction, Name, IP, Verdict).
...
measure_response(#state{name = Name},
    ...
    smf_prometheus:pfcp_peer_response(Name, IP, Msg, SendTS - ArrivalTS).
...
measure_request(#state{name = Name},
    ...
    smf_prometheus:pfcp_request_duration(Name, MsgType, Duration).
