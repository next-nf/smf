...
handle_request(ReqKey, IP, #pfcp{type = heartbeat_request} = Request) ->
    case smf_sx_node_reg:lookup(IP) of
...
handle_request_fun(ReqKey, #pfcp{type = session_report_request} = Report) ->
    {Ctx, IEs} =
	case smf_context:sx_report(Report) of
	    {ok, Ctx0} ->
    ...
    smf_sx_socket:send_response(ReqKey, Response, true),
    ok.

set_defaults(Opts0) ->
    Opts = smf_sx_node:validate_defaults(Opts0),
    smf_core_config:put(up_node_defaults, Opts).

set_required_upff(Opts) when is_map(Opts) ->
    smf_core_config:put(up_required_feature, Opts);
set_required_upff(Opts) ->
    error(badarg, [Opts]).

add_sx_node(Name, Opts0) ->
    Opts = validate_options(Name, Opts0),
    {ok, Nodes} = smf_core_config:get([nodes], #{}),
    smf_core_config:put(nodes, maps:put(Name, Opts, Nodes)),
    {ok, Defaults} = smf_core_config:get([up_node_defaults], #{connect => false}),
...
connect_sx_node(Node, #{raddr := IP4} = _Opts) when tuple_size(IP4) =:= 4 ->
    smf_sx_node_reg:connect(Node, default, [IP4], []);
connect_sx_node(Node, #{raddr := IP6} = _Opts) when tuple_size(IP6) =:= 8 ->
    smf_sx_node_reg:connect(Node, default, [], [IP6]);
connect_sx_node(Node, #{node_selection := NodeSelect} = Opts) ->
    case smf_node_selection:lookup(Node, NodeSelect) of
	{_, IP4, IP6} ->
	    smf_sx_node_reg:connect(Node, NodeSelect, IP4, IP6);
	_Other ->
	    %% TBD
...
validate_node_vrfs({Name, Opts}) when is_map(Opts) ->
    {vrf:validate_name(Name),
     smf_core_config:validate_options(fun validate_node_vrf_option/2, Opts, ?VrfDefaults)};
validate_node_vrfs({Name, Opts}) when is_list(Opts) ->
    validate_node_vrfs({Name, smf_core_config:to_map(Opts)});
...
validate_node_ue_ip_pool(Pool) when is_map(Pool) ->
    smf_core_config:validate_options(fun validate_node_ue_ip_pool/2, Pool, ?UeIpPoolDefaults);
validate_node_ue_ip_pool(Pool) when is_list(Pool) ->
    validate_node_ue_ip_pool(smf_core_config:to_map(Pool));
validate_node_ue_ip_pool(Pool) ->
    erlang:error(badarg, [Pool]).

validate_node_ue_ip_pool(ip_pools, Names) when is_list(Names) ->
    V = [smf_ip_pool:validate_name(ip_pools, Name) || Name <- Names],
    smf_core_config:check_unique_elements(ip_pools, V),
    V;
...
validate_node_default_option(vrfs, VRFs)
  when ?is_non_empty_opts(VRFs) ->
    smf_core_config:validate_options(fun validate_node_vrfs/1, VRFs, []);
...
validate_node_default_option(request, Opts)
  when ?is_opts(Opts) ->
    smf_core_config:validate_options(fun validate_node_request/1, Opts, ?NodeDefaultRequest);
...
init([Parent, Node, NodeSelect, IP4, IP6, NotifyUp]) ->
    ...
    {ok, CP, Socket, SockInfo} = smf_sx_socket:id(),
    {ok, TEI} = smf_tei_mngr:alloc_tei(Socket),
    SEID = smf_sx_socket:seid(),
    ...
    {ok, ReqUpFF} =
	smf_core_config:get([up_required_feature], #{}),
    {ok, Default} = smf_core_config:get([up_node_defaults], #{}),
    Cfg1 = case smf_core_config:get([nodes, Node], Default) of
	      {ok, Cfg0} -> Cfg0;
	      _ -> Default
	  end,
...
handle_event(enter, _OldState, {connected, ready},
    ...
    ok = smf_sx_node_reg:up(Node, NodeCaps),
...
handle_event(cast, {send, 'Access', _VRF, Data}, {connected, _},
    ...
    smf_gtp_u_socket:send(Socket, gtp, IP, ?GTP1u_PORT, Bin),
...
handle_event(cast, {response, from_cp_rule,
    ...
    PCtx1 = smf_pfcp_context:update_dp_seid(IEs, PCtx0),
    {Bearer, PCtx} = smf_pfcp_context:update_bearer(IEs, Bearer0, PCtx1),
    smf_pfcp_context:register_ctx_ids(?MODULE, Bearer, PCtx),
...
handle_event({call, From}, attach, _, #data{pfcp_ctx = PNodeCtx} = Data) ->
    PCtx = #pfcp_ctx{
	      ...
	      seid = #seid{cp = smf_sx_socket:seid()},
...
handle_event({call, _} = Evt, #pfcp{} = Request0, {connected, _},
    ...
    smf_sx_socket:call(IP, Timeout, Retry, Request, response_cb(Evt)),
    keep_state_and_data;
...
make_request(IP, Port, Msg, #data{cp_socket = Socket, cp_info = Info}) ->
    smf_gtp_socket:make_request(0, sx, smf_inet:bin2ip(IP), Port, Msg, Socket, Info).
...
handle_udp_gtp(SrcIP, DstIP, <<SrcPort:16, DstPort:16, _:16, _:16, PayLoad/binary>>,
  ...
  when DstPort =:= ?GTP1u_PORT ->
    ...
    TEID = #fq_teid{ip = smf_inet:bin2ip(DstIP), teid = Msg#gtp.tei},
    smf_context:port_message(gtp_context:socket_teid_key(Socket, TEID), ReqKey, Msg, false),
    ok;
...
request_connect(Candidates, NodeSelect, AbsTimeout, Ref, Owner) ->
    Available = smf_sx_node_reg:available(),
...
connect_node({Node, _, _, IP4, IP6}, NodeSelect, _Available, Expects) ->
    ...
    smf_sx_node_reg:connect(Node, NodeSelect, IP4, IP6, [NotifyUp]),
    [NotifyUp | Expects].
...
put_build_id(R = #pfcp{ie = IEs}) ->
    {ok, Version} = application:get_key(smf_core, vsn),
...
put_recovery_time_stamp(R = #pfcp{ie = IEs}) ->
    TS = #recovery_time_stamp{
	    time = smf_gsn_lib:seconds_to_sntp_time(gtp_config:get_start_time())},
...
send_heartbeat(#data{dp = #node{ip = IP}, cfg = #{heartbeat := #{timeout := Timeout, retry := Retry}}}) ->
    ...
    smf_sx_socket:call(IP, Timeout, Retry, Req, response_cb(heartbeat)).
...
handle_nodedown(#data{pfcp_ctx = PCtx, bearer = Bearer} = Data) ->
    smf_pfcp_context:unregister_ctx_ids(?MODULE, Bearer, PCtx),
    Self = self(),
...
assign_local_data_teid(Key, PCtx, #node{ip = NodeIP}, VRFs, Bearer) ->
    ...
    smf_gsn_lib:assign_local_data_teid(Key, PCtx, VRF, NodeIP, Bearer).
...
generate_per_feature_pfcp_rule('Access', #vrf{name = Name} = VRF, Bearer, PCtx0) ->
    Key = {Name, 'Access'},
    {PdrId, PCtx1} = smf_pfcp:get_id(pdr, Key, PCtx0),
    {FarId, PCtx2} = smf_pfcp:get_id(far, Key, PCtx1),
    {BearerReq, PCtx} = smf_pfcp_context:make_request_bearer(PdrId, Bearer, PCtx2),

    %% GTP-U encapsulated packet from CP
    PDI = smf_pfcp:traffic_endpoint(BearerReq, #{}),
    PDR0 = #{pdr_id => #pdr_id{id = PdrId},
	     precedence => #precedence{precedence = 100},
	     pdi => PDI,
	     far_id => #far_id{id = FarId}},
    PDR = smf_pfcp:outer_header_removal(Bearer, PDR0),
...
install_cp_rules(#data{pfcp_ctx = PCtx0,
    ...
    PCtx1 = smf_pfcp:init_ctx(PCtx0),
    PCtx = maps:fold(generate_pfcp_rules(_, _, DpBearer, _), PCtx1, VRFs),
    Rules = smf_pfcp:update_pfcp_rules(PCtx1, PCtx, #{}),
    IEs = update_m_rec(smf_pfcp:f_seid(PCtx, CntlNode), Rules),

    Req0 = #pfcp{version = v1, type = session_establishment_request, seq_no = 0, seid = 0, ie = IEs},
    Req = augment_mandatory_ie(Req0, Data),
    smf_sx_socket:call(DpNodeIP, Timeout, Retry, Req, response_cb(from_cp_rule)),
