...
-export([init_ctx/1, reset_ctx/1,
	 get_id/3, get_chid/3, get_bearer_handle_by_pdr/2,
	 update_pfcp_rules/3,
	 update_teids/3]).
...
alloc_info_addr(AI) ->
    smf_inet:ip2bin(smf_ip_pool:addr(AI)).
...
f_seid(#pfcp_ctx{seid = #seid{cp = SEID}}, #node{ip = {_,_,_,_} = IP}) ->
   #f_seid{seid = SEID, ipv4 = smf_inet:ip2bin(IP)};
f_seid(#pfcp_ctx{seid = #seid{cp = SEID}}, #node{ip = {_,_,_,_,_,_,_,_} = IP}) ->
    #f_seid{seid = SEID, ipv6 = smf_inet:ip2bin(IP)}.
...
outer_header_creation(#fq_teid{ip = {_,_,_,_} = IP, teid = TEID}) ->
    #outer_header_creation{type = 'GTP-U', teid = TEID, ipv4 = smf_inet:ip2bin(IP)};
outer_header_creation(#fq_teid{ip = {_,_,_,_,_,_,_,_} = IP, teid = TEID}) ->
    #outer_header_creation{type = 'GTP-U', teid = TEID, ipv6 = smf_inet:ip2bin(IP)}.
...
update_teids(Id, FqTEID, {pdr, _},
	     #{pdr_id := #pdr_id{id = Id}, pdi := PDI} = Rules)
  when is_map_key(f_teid, PDI) ->
    maps:put(pdi, maps:put(f_teid, FqTEID, PDI), Rules);
