...
init({Name, #{ip := IP, vrf := VRF, burst_size := BurstSize} = SocketOpts}) ->
    ...
    {ok, Socket} = smf_gtp_socket:make_gtp_socket(IP, ?GTP1u_PORT, SocketOpts),
    ...
    smf_socket_reg:register('gtp-u', Name, GtpSocket),
...
handle_cast(#send_req{address = IP, port = Port, msg = Msg},
    ...
    smf_prometheus:gtp(tx, Socket, IP, Msg),
...
make_request(IP, Port, Msg, #state{gtp_socket = Socket, info = Info}) ->
    smf_gtp_socket:make_request(0, gtp, IP, Port, Msg, Socket, Info).
