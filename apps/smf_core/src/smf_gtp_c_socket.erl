...
-module(smf_gtp_c_socket).

-behavior(gen_server).
-behavior(smf_gtp_socket).
...
init({Name, #{ip := IP, vrf := VRF,
    ...
    {ok, RecvSocket} = smf_gtp_socket:make_gtp_socket(IP, ?GTP1c_PORT, SocketOpts),
    SendSocket =
	case SendPort of
	    _ when is_integer(SendPort) ->
		{ok, SndSocket} = smf_gtp_socket:make_gtp_socket(IP, SendPort, SocketOpts),
    ...
    smf_socket_reg:register('gtp-c', Name, GtpSocket),
    ...
    State = #state{
	       ...
	       requests = smf_cache:new(?CACHE_TIMEOUT_INTERVAL, requests),
	       responses = smf_cache:new(?CACHE_TIMEOUT_INTERVAL, responses),
...
handle_call(get_request_q, _From, #state{requests = Requests} = State) ->
    {reply, smf_cache:to_list(Requests), State};
handle_call(get_response_q, _From, #state{responses = Responses} = State) ->
    {reply, smf_cache:to_list(Responses), State};
...
handle_info({timeout, TRef, requests}, #state{requests = Requests} = State) ->
    {noreply, State#state{requests = smf_cache:expire(TRef, Requests)}};

handle_info({timeout, TRef, responses}, #state{responses = Responses} = State) ->
    {noreply, State#state{responses = smf_cache:expire(TRef, Responses)}};
...
make_request(ArrivalTS, Src, IP, Port, Msg, #state{gtp_socket = Socket, info = Info}) ->
    smf_gtp_socket:make_request(ArrivalTS, Src, IP, Port, Msg, Socket, Info).
...
handle_message(ArrivalTS, Src, IP, Port, Data, #state{gtp_socket = Socket} = State0) ->
	...
	Msg = #gtp{} ->
	    ...
	    ?LOG(debug, "handle message: ~p", [{Src, IP, Port,
						...
						Msg}]),
	    smf_prometheus:gtp(rx, Socket, IP, Msg),
	    handle_message_1(ArrivalTS, Src, IP, Port, Msg, State0)
    catch
	Class:Error ->
	    smf_prometheus:gtp_error(rx, Socket, 'malformed-requests'),
...
handle_message_1(_, _, _, _, #gtp{version = Version}, #state{gtp_socket = Socket} = State)
  when Version /= v1 andalso Version /= v2 ->
    smf_prometheus:gtp_error(rx, Socket, 'version-not-supported'),
    State;
...
handle_response(ArrivalTS, _Src, IP, _Port, Msg, #state{gtp_socket = Socket} = State0) ->
    SeqId = smf_gtp_socket:make_seq_id(Msg),
    {Req, State} = take_request(SeqId, State0),
    case Req of
	none -> %% duplicate, drop silently
	    smf_prometheus:gtp(rx, Socket, IP, Msg, duplicate),
...
handle_request(#request{src = Src, ip = IP, port = Port} = ReqKey, Msg,
    ...
    case smf_cache:get(cache_key(ReqKey), Responses) of
	{value, Data} ->
	    smf_prometheus:gtp(rx, Socket, IP, Msg, duplicate),
	    sendto(Src, IP, Port, Data, State);
	...
	_Other ->
	    ...
	    smf_context:port_message(ReqKey, Msg)
    end,
    State.
...
request_q_in(#send_req{req_id = ReqId} = SendReq, #state{requests = Requests} = State) ->
    State#state{requests = smf_cache:enter(cache_key(ReqId), SendReq, ?RESPONSE_TIMEOUT, Requests)}.

request_q_peek(ReqId, #state{requests = Requests}) ->
    smf_cache:get(cache_key(ReqId), Requests).

enqueue_response(ReqKey, Data, DoCache,
		 #state{responses = Responses} = State)
  when DoCache =:= true ->
    State#state{responses = smf_cache:enter(cache_key(ReqKey), Data, ?RESPONSE_TIMEOUT, Responses)};
...
message_counter(Direction, Socket, #send_req{address = IP, msg = Msg}) ->
    smf_prometheus:gtp(Direction, Socket, IP, Msg);
...
message_counter(Direction, Socket, #send_req{address = IP, msg = Msg}, Verdict) ->
    smf_prometheus:gtp(Direction, Socket, IP, Msg, Verdict).
...
measure_reply(Socket, #send_req{address = IP, msg = Msg, send_ts = SendTS}, ArrivalTS) ->
    smf_prometheus:gtp_path_rtt(Socket, IP, Msg, ArrivalTS - SendTS).
...
measure_response(#request{
    ...
    smf_prometheus:gtp_request_duration(Socket, Version, MsgType, Duration).
