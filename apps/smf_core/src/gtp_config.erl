...
init() ->
    StateFile = setup:get_env(?App, state_file, filename:join(setup:data_dir(), "smf.state")),
...
sync() ->
    ...
    State = [{restart_count, smf_global:get(restart_count)}],
