...
add(Name0, Opts0) ->
    ...
    {ok, APNs} = smf_core_config:get([apns], #{}),
    smf_core_config:put(apns, maps:put(Name, Opts, APNs)).
...
get([H|_] = APN0, APNs) when is_binary(H) ->
    ...
    {NI, OI} = smf_node_selection:split_apn(APN),
    FqAPN = NI ++ OI,
    case APNs of
	#{FqAPN := A} -> {ok, A};
	#{NI :=    A} -> {ok, A};
...
validate_options({APN0, Value}) when ?is_opts(Value) ->
    ...
    Opts = smf_core_config:validate_options(fun validate_apn_option/1, Value, ?ApnDefaults),
...
validate_apn_option({vrfs = Opt, VRFs})
  when length(VRFs) /= 0 ->
    ...
    smf_core_config:check_unique_elements(Opt, V),
    {Opt, V};
validate_apn_option({ip_pools = Opt, Pools})
  when is_list(Pools) ->
    V = [smf_ip_pool:validate_name(Opt, Name) || Name <- Pools],
    smf_core_config:check_unique_elements(Opt, V),
    {Opt, V};
...
validate_apn_option({Opt, Value})
  when Opt == 'MS-Primary-DNS-Server';   Opt == 'MS-Secondary-DNS-Server';
    ...
    {Opt, smf_core_config:validate_ip_cfg_opt(Opt, Value)};
