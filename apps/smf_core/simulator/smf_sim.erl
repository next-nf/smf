...
-define(CONFIG,
	[
	 ...
	 {smf_aaa, [{smf_aaa_provider, {smf_aaa_mock, [{shared_secret, <<"MySecret">>}]}}]}
	]).
...
start(CpIP0, UpIP0) ->
    ...
    {ok, _} = application:ensure_all_started(smf),

    CpOpts = #{type => 'gtp-c',
	       ip   => CpIP,
	       freebind => true},
    {ok, _} = smf:start_socket('gtp-c', CpOpts),
    {ok, _} = smf_sim_up:start_socket(UpIP),
    ok.
...
s11(IP, initial_attachment, APN, IMEI, IMSI, MSISDN, PdnType) ->
    CntlPort = #gtp_port{ip = LocalIP} = gtp_socket_reg:lookup('gtp-c'),
    {ok, CpTEI} = smf_tei_mngr:alloc_tei(CntlPort),
    {ok, DpTEI} = smf_tei_mngr:alloc_tei(CntlPort),
    Peer = parse_ip(IP),
    SeqNo = 1,
